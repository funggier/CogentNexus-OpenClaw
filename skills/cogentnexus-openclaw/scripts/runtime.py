...
def lifecycle_cmd(args):
    ...
    if args.command_name == "status":
        emit({"maintenance": marker, "gateway": gateway_probe(int(config["supervisor"]["commandTimeoutSeconds"])), "ollama": ollama_probe(config), "startupHint": "After login, native OpenClaw startup plus the CogentNexus-OpenClaw supervisor normally restores runtime; use lifecycle start for an immediate verified start."})
        return 0
...
def monitor_workflows(root, config, execute=False):
    workspace = root.parent if root.name == ".cogentnexus-openclaw" else root
...
def render_template(backend, root):
    names = {"windows": "windows-task.xml", "systemd": "cogentnexus-openclaw-supervisor.service", "systemd-timer": "cogentnexus-openclaw-supervisor.timer", "launchd": "ai.cogentnexus.openclaw.supervisor.plist", "cron": "cron.txt", "docker": "docker-compose.yml", "kubernetes": "kubernetes-probes.yaml"}
