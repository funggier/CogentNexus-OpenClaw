...
async function maintain(api:any,row:Maintenance,config:ContextGuardConfig,workspaceDir:string,databasePath:string) {
  ...
  // caused it and CNX generation remained unchanged. Otherwise a UI lifecycle
  ...
  registrationApi.registerService?.({id:"cogentnexus-context-maintenance-v090",start:async(ctx:any)=>{
    ...
    const tick=async()=>{if(active)return;active=true;try{
      for(const row of due(databasePath,maxAttempts)){
        ...
        try{const result=await maintain(api,row,config,workspaceDir,databasePath);api.logger.info?.(`CogentNexus context maintenance ${row.session_key}: ${result.action}`);}
