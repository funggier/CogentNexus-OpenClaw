...
describe("cogentnexus-openclaw", () => {
  ...
  it("does not capture simple or internal continuation turns", () => {
    ...
    expect(classifyDurableRequest("CogentNexus-OpenClaw workflow X reached terminal status completed.").lane).toBe("direct");
  ...
  it("compiles a bounded owner-startable manifest without invoking a model", () => {
    ...
    expect(manifest.steps[0].executor.includeFiles).toEqual([`.cogentnexus-openclaw/intake/${manifest.taskId}/request.txt`]);
  ...
  it("deduplicates the same active durable request across owner sessions", () => {
    ...
    const flow = join(root, ".cogentnexus-openclaw", "workflows", "WF-ACTIVE");
  ...
  it("declares the rotation tool", () => {
    expect(getToolPluginMetadata(entry)?.tools.map((tool) => tool.name)).toEqual(["cnxclaw_rotation", "cnxclaw_workflow_start", "cnxclaw_ticket_status", "cnxclaw_knowledge", "cnxclaw_research"]);
  });
  ...
  it("blocks conversational unbound workflow creation but permits the trusted start tool", () => {
    ...
    expect(enforcementDecision("cnxclaw_workflow_start", {manifestPath:"manifest.json"}, "agent:main:owner").block).toBe(false);
  ...
  it("schedules a post-compaction continuation only while durable session work remains", async () => {
    ...
    try {
      ...
      expect(String(scheduled[0].message)).toContain("[CogentNexus-OpenClaw Continuation: post-compaction]");
  ...
  it("discovers only pending terminal workflow outboxes", () => {
    ...
    try {
      const base = join(root, ".cogentnexus-openclaw", "workflows", "WF-1");
  ...
  it("finishes TaskFlow and schedules the owner while delivery remains pending until receipt", async () => {
    ...
    try {
      ...
      expect(String(scheduled[0].message)).toContain("[CogentNexus-OpenClaw Delivery: workflow:WF-2:9]");
  ...
  it("compiles, starts, links, heartbeats, and completes an admitted Ticket workflow", () => {
    ...
    try {
      ...
      const result = dispatchTicketWorkflows({workspaceDir:root,store,config:{ticketLeaseMs:5000} as any,now,
        ...
        compile:(input:any)=>{compiled.push(input);return {taskId:"WF-BRIDGE",manifestPath:".cogentnexus-openclaw/intake/WF-BRIDGE/manifest.json",componentCount:3,assembledOutput:"out",requestHash:"hash"};},
      ...
      expect(started[0]).toMatchObject({ownerSessionKey:"agent:main:owner",manifestPath:".cogentnexus-openclaw/intake/WF-BRIDGE/manifest.json"});
      expect(store.get(ticket.ticketId)).toMatchObject({workflowId:"WF-BRIDGE",manifestPath:".cogentnexus-openclaw/intake/WF-BRIDGE/manifest.json"});

      const workflow = join(root,".cogentnexus-openclaw","workflows","WF-BRIDGE");
  ...
  it("reclaims a killed worker and relinks the existing workflow after service restart", async () => {
    ...
    try {
      ...
      const workflow = join(root,".cogentnexus-openclaw","workflows",intake.taskId);
      mkdirSync(workflow,{recursive:true});
      ...
      expect(store.get(ticket.ticketId)).toMatchObject({workflowId:intake.taskId,manifestPath:`.cogentnexus-openclaw/workflows/${intake.taskId}/manifest.json`});
