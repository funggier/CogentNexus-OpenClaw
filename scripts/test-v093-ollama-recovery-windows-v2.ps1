...
if ($SyntaxOnly) {
    Write-Host 'CogentNexus-OpenClaw v0.9.3 Ollama Recovery Reality harness v2 syntax/load: PASS'
    exit 0
}
...
$LogPath = Join-Path $Downloads "CNXCLAW_V093_OLLAMA_RECOVERY_V2_$Stamp.txt"
$JsonPath = Join-Path $Downloads "CNXCLAW_V093_OLLAMA_RECOVERY_V2_$Stamp.json"
...
Set-Content -Path $LogPath -Value "CogentNexus-OpenClaw v0.9.3 Ollama-only Recovery Reality Windows Harness v2`r`n" -Encoding UTF8; Save-Evidence
try {
