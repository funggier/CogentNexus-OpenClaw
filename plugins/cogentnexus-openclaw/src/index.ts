...
function workflowRuntime(workspaceDir: string): string {
  const path = resolve(workspaceDir, "skills", "cogentnexus-openclaw", "scripts", "workflow.py");
  if (!existsSync(path)) throw new Error(`CogentNexus-OpenClaw workflow runtime not found: ${path}`);
  return path;
}
...
export function startBoundWorkflow(input: { workspaceDir:string; manifestPath:string; ownerSessionKey:string; pythonCommand?:string }) {
  ...
  try {
    ...
    const flowDir = typeof taskId === "string" ? resolve(workspace,".cogentnexus-openclaw","workflows",taskId) : "";
  ...
  const flowDir = resolve(workspace,".cogentnexus-openclaw","workflows",taskId);
...
export function dispatchTicketWorkflows(input:{workspaceDir:string;store:TicketStore;config:RotationConfig;now?:Date;snapshot?:TicketResourceSnapshot;
  ...
  const leases = dispatcher.dispatch({
    ...
    launch:(lease)=>{
      ...
      const manifestPath = intake?.manifestPath ?? `.cogentnexus-openclaw/workflows/${started.taskId}/manifest.json`;
...
export function reconcileTicketWorkflows(input:{workspaceDir:string;store:TicketStore;config:RotationConfig;now?:Date}) {
  ...
  for (const linked of input.store.linkedRunning()) {
    const statePath = resolve(input.workspaceDir,".cogentnexus-openclaw","workflows",linked.workflowId,"state.json");
...
export function completionMessage(notice: WorkflowCompletion): string {
  return [
    ...
    `CogentNexus-OpenClaw workflow ${notice.taskId} reached terminal status ${notice.workflowStatus}.`,
...
export async function deliverWorkflowCompletion(api: any, path: string, notice: WorkflowCompletion) {
  ...
  try {
    ...
    await api.session.workflow.scheduleSessionTurn({sessionKey:notice.ownerSessionKey,delayMs:1000,deleteAfterRun:true,deliveryMode:"announce",
      name:`CogentNexus-OpenClaw workflow ${notice.taskId}`,tag,message:completionMessage(notice)});
    return scheduled;
  } catch (error) {
...
export async function deliverTicketOutbox(api: any, store: TicketStore, item: TicketOutbox) {
  ...
  try {
    ...
    await api.session.workflow.scheduleSessionTurn({
      ...
      name:`CogentNexus-OpenClaw Ticket ${item.ticketId}`,tag,
      message:[ticketDeliveryMarker(item.outboxId),`CogentNexus-OpenClaw Ticket ${item.ticketId} reached terminal status ${item.terminalStatus}.`,
...
export function enforcementDecision(toolName: string, params: Record<string, unknown>, sessionKey?: string, enabled = true) {
  if (!enabled || !sessionKey || toolName === "cnxclaw_workflow_start") return { block:false };
...
export function monitorRotations(workspaceDir: string, config: RotationConfig, sessionKey: string): string[] {
  const phase3Path = resolve(workspaceDir, "skills", "cogentnexus-openclaw", "scripts", "phase3.py");
  const cogentNexusOpenClawRoot = resolve(config.cogentNexusOpenClawRoot ?? join(workspaceDir, ".cogentnexus-openclaw"));
  const result = spawnSync(config.pythonCommand ?? "python", [phase3Path, "--root", cogentNexusOpenClawRoot, "context", "monitor", "--execute-safe"], {
...
export async function schedulePostCompactionResume(input: {
  ...
  await input.workflow.scheduleSessionTurn({
    ...
    deleteAfterRun:true,
    deliveryMode:"announce",
    name:"CogentNexus-OpenClaw post-compaction continuation",
    tag,
    message:[
      "#cogent-direct",
      "[CogentNexus-OpenClaw Continuation: post-compaction]",
      "Compaction completed while CogentNexus-OpenClaw still has non-terminal work or unconfirmed delivery for this session.",
...
export async function scheduleInterruptedResume(input: {
  ...
  await input.workflow.scheduleSessionTurn({
    ...
    deleteAfterRun: true,
    deliveryMode: "announce",
    name: `CogentNexus-OpenClaw resume ${runId.slice(0, 12)}`,
    tag,
    message: [
      "The previous run was interrupted.",
      "Resume automatically from committed CogentNexus-OpenClaw durable state and the latest valid handoff.",
...
export function inspectHandoff(
...
): { handoff: Handoff; phase3Path: string; cogentNexusOpenClawRoot: string } {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(taskId)) {
    throw new Error("taskId contains unsupported characters");
  }
  const phase3Path = resolve(workspaceDir, "skills", "cogentnexus-openclaw", "scripts", "phase3.py");
  const cogentNexusOpenClawRoot = resolve(config.cogentNexusOpenClawRoot ?? join(workspaceDir, ".cogentnexus-openclaw"));
  if (!existsSync(phase3Path)) throw new Error(`CogentNexus-OpenClaw phase3 runtime not found: ${phase3Path}`);
  const result = spawnSync(config.pythonCommand ?? "python", [
    phase3Path, "--root", cogentNexusOpenClawRoot, "context", "inspect", "--task-id", taskId,
  ], { encoding: "utf8", windowsHide: true, timeout: 30_000 });
  if (result.error) throw result.error;
  ...
  return { handoff, phase3Path, cogentNexusOpenClawRoot };
}
...
function workerPrompt(taskId: string, phase3Path: string, cogentNexusOpenClawRoot: string): string {
  return [
    "Resume a CogentNexus-OpenClaw task from durable state; do not reconstruct prior private reasoning.",
    `Task ID: ${taskId}`,
    `Cogent root: ${cogentNexusOpenClawRoot}`,
    `Runtime: ${phase3Path}`,
...
async function startRotation(api: any, taskFlow: any, ownerSessionKey: string, workspaceDir: string, config: RotationConfig, taskId: string) {
  ...
  const cliArgs = ["agent","--session-key",identity.childSessionKey,"--message",workerPrompt(taskId,checked.phase3Path,checked.cogentNexusOpenClawRoot),"--json","--timeout",String(config.timeoutSeconds ?? 3600)];
...
const configSchema = Type.Object({
  ...
  workspaceDir: Type.Optional(Type.String({ description: "Workspace containing CogentNexus-OpenClaw durable state and workflow completion outboxes." })),
...
const entry = defineToolPlugin({
  ...
  description: "Ticket-first OpenClaw bridge for CogentNexus-OpenClaw Host-managed continuity, durable execution, recovery, context handoff, and verified delivery.",
  configSchema,
  tools: (tool) => [tool({
    name: "cnxclaw_rotation",
    label: "CogentNexus-OpenClaw Rotation",
    description: "Plan or start a safe session rotation. Defaults to dry-run; execute only after CogentNexus-OpenClaw produced a verified ROTATE handoff.",
    parameters: Type.Object({
      taskId: Type.String({ description: "Durable CogentNexus-OpenClaw task id." }),
    ...
    factory: ({ api, config, toolContext }) => ({
      name: "cnxclaw_rotation",
      label: "CogentNexus-OpenClaw Rotation",
      description: "Plan or start a safe CogentNexus-OpenClaw session rotation.",
      parameters: Type.Object({
        taskId: Type.String(),
      ...
      name:"cnxclaw_workflow_start",
      label:"Start CogentNexus-OpenClaw Workflow",
      ...
      factory:({config,toolContext})=>({
        name:"cnxclaw_workflow_start",label:"Start CogentNexus-OpenClaw Workflow",description:"Start an owner-bound durable workflow.",
      ...
      name:"cnxclaw_ticket_status",
      label:"CogentNexus-OpenClaw Ticket Status",
      ...
      name:"cnxclaw_knowledge",
      label:"CogentNexus-OpenClaw Knowledge",
      ...
      optional:true,
      factory:({config,toolContext})=>({
        name:"cnxclaw_knowledge",label:"CogentNexus-OpenClaw Knowledge",description:"Use the durable evidence-backed knowledge store.",
        ...
        async execute(_id:string,params:any) {
          if(config.knowledgeEnabled === false) throw new Error("CogentNexus-OpenClaw knowledge capability is disabled");
      ...
      name:"cnxclaw_research",
      label:"CogentNexus-OpenClaw External Research",
      ...
      optional:true,
      factory:({config,toolContext})=>({
        name:"cnxclaw_research",label:"CogentNexus-OpenClaw External Research",description:"Use the bounded external-research evidence store.",
        ...
        async execute(_id:string,params:any){
          if(config.externalResearchEnabled === false) throw new Error("CogentNexus-OpenClaw external research capability is disabled");
...
entry.register = (api) => {
  ...
  if (config.ticketFirst === true) api.on("session_end", (event, ctx) => {
    ...
    try {
      ...
      api.logger.info?.(`CogentNexus-OpenClaw rebound session continuity ${event.sessionKey} -> ${event.nextSessionKey}: ${rebound.ticketIds.length} tickets, ${rebound.outboxCount} pending outbox deliveries`);
    } catch(error) { api.logger.error(`CogentNexus-OpenClaw session successor rebind failed: ${error instanceof Error?error.message:String(error)}`); }
  ...
  if (config.autoResume !== false && config.ticketFirst === true) api.on("after_compaction", async (_event, ctx) => {
    ...
    } catch(error) { api.logger.warn(`CogentNexus-OpenClaw post-compaction continuation scheduling failed: ${error instanceof Error?error.message:String(error)}`); }
  ...
  if (config.autoResume !== false || config.autoRotate === true || config.ticketFirst === true) api.on("agent_end", async (event, ctx) => {
    ...
    if (event.success && config.autoRotate === true && sessionKey) {
      try {
        ...
        api.logger.warn(`CogentNexus-OpenClaw automatic rotation skipped: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
        ...
        const tick = async () => {
          ...
          try {
            for (const item of pendingWorkflowCompletions(workspaceDir,new Date(),config.outboxDeliveryTimeoutMs ?? 300000)) {
              ...
              catch (error) { api.logger.warn(`CogentNexus-OpenClaw workflow completion delivery failed for ${item.notice.taskId}: ${error instanceof Error ? error.message : String(error)}`); }
            }
      ...
      start: async (ctx: any) => {
        ...
        const tick = async () => {
          ...
          try {
            ...
            for (const item of undelivered) api.logger.warn(`CogentNexus-OpenClaw promoted unconfirmed direct delivery ${item.ticketId} (${item.runId}) to durable recovery`);
            const recovered = store.recoverExpired();
            for (const item of recovered) api.logger.warn(`CogentNexus-OpenClaw recovered expired Ticket ${item.ticketId} from worker ${item.previousWorkerId ?? "unknown"} generation ${item.previousLeaseGeneration}`);
            for (const item of reconcileTicketWorkflows({workspaceDir,store,config})) api.logger.info?.(`CogentNexus-OpenClaw Ticket ${item.ticketId} workflow action ${item.action}`);
            const dispatched = dispatchTicketWorkflows({workspaceDir,store,config});
            if (!dispatched.admission.admitted) api.logger.info?.(`CogentNexus-OpenClaw Ticket dispatch deferred: ${dispatched.admission.reasons.join(",")}`);
            for (const item of store.pendingOutbox(100,new Date(),config.outboxDeliveryTimeoutMs ?? 300000)) {
              ...
              catch (error) { api.logger.warn(`CogentNexus-OpenClaw Ticket completion delivery failed for ${item.ticketId}: ${error instanceof Error ? error.message : String(error)}`); }
            }
          } catch (error) {
            api.logger.error(`CogentNexus-OpenClaw Ticket recovery scan failed: ${error instanceof Error ? error.message : String(error)}`);
          } finally { active = false; }
        };
        await tick();
