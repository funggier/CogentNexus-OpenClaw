...
if ($SyntaxOnly) {
    Write-Host 'CogentNexus v0.9.3 Recovery Reality harness v2 syntax/load: PASS'
    exit 0
}
...
$LogPath = Join-Path $Downloads "CNX_V093_RECOVERY_REALITY_V2_$Stamp.txt"
$JsonPath = Join-Path $Downloads "CNX_V093_RECOVERY_REALITY_V2_$Stamp.json"
$RunRoot = Join-Path $Downloads "CNX_V093_RECOVERY_REALITY_V2_$Stamp"
...
$Evidence = [ordered]@{
    schemaVersion = 2
    suite = 'v0.9.3-recovery-reality-windows-v2'
...
function Save-Evidence {
    $Evidence | ConvertTo-Json -Depth 60 | Set-Content -Path $JsonPath -Encoding UTF8
}
...
function Clear-ActiveOperation {
    $Evidence.activeOperation = $null
    Save-Evidence
}
...
function Invoke-CapturedProcess {
    ...
    $stdout = Join-Path $env:TEMP "cnx-v093v2-$Stamp-$([guid]::NewGuid().ToString('N')).out.txt"
    $stderr = Join-Path $env:TEMP "cnx-v093v2-$Stamp-$([guid]::NewGuid().ToString('N')).err.txt"
    $launchFile = $FilePath
    $launchArgs = @($Arguments)
    $ext = [IO.Path]::GetExtension($FilePath)
    if ($ext -ieq '.cmd' -or $ext -ieq '.bat') {
        $cmdline = (Quote-CmdArg $FilePath)
        foreach ($a in $Arguments) { $cmdline += ' ' + (Quote-CmdArg ([string]$a)) }
        $launchFile = 'cmd.exe'
    ...
    try {
        ...
        Set-ActiveOperation $Name ([ordered]@{
            mode='captured-process'
            requestedFile=$FilePath
            requestedArguments=@($Arguments)
            launchFile=$launchFile
            launchArguments=@($launchArgs)
            childPid=[int]$proc.Id
            child=Get-ProcessRecord -Id ([int]$proc.Id)
            timeoutSeconds=$TimeoutSeconds
        })
        if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
            Write-Evidence "TIMEOUT $Name :: stopping exact root pid=$($proc.Id); no tree kill"
...
function Get-ProviderPort { if ($Provider -eq 'lmstudio') { return 1234 } return 11434 }
...
function Assert-SafeKillTarget {
    param([string]$Role,$Snapshot)
    if (-not [bool]$Snapshot.listening -or $null -eq $Snapshot.pid -or $null -eq $Snapshot.process) { throw "$Role kill target is not a live listener process." }
    ...
    $path = ([string]$target.executablePath).ToLowerInvariant()
    $banned = @('powershell.exe','pwsh.exe','cmd.exe','conhost.exe','firefox.exe','explorer.exe','windowsterminal.exe','openconsole.exe')
    if ($banned -contains $name) { throw "Refusing to kill unsafe process name '$name' for role $Role." }
    if ($targetPid -eq [int]$PID) { throw "Refusing to kill harness PID $targetPid." }
    $ancestors = @(Get-AncestorPids -StartPid ([int]$PID))
    if ($ancestors -contains $targetPid) { throw "Refusing to kill ancestor PID $targetPid for role $Role." }
    $identityOkay = $false
    if ($Role -eq 'gateway') {
        $identityOkay = ($name -eq 'node.exe' -and ($cmd -match 'openclaw' -or $path -match 'node'))
    } elseif ($Role -eq 'provider') {
        if ($Provider -eq 'lmstudio') { $identityOkay = (($name -match 'llama|lms|lm studio') -or ($cmd -match 'lmstudio|llama')) }
        if ($Provider -eq 'ollama') { $identityOkay = (($name -match 'ollama') -or ($cmd -match 'ollama')) }
    }
    if (-not $identityOkay) { throw "Refusing to kill unverified $Role target pid=$targetPid name='$name' commandLine='$($target.commandLine)'." }
...
function Get-JsonCommand {
    ...
    $r = Invoke-CapturedProcess $Name $Cnx $Args $Allowed 900
    try { return $r.Stdout | ConvertFrom-Json } catch { throw "$Name did not return valid JSON." }
}
...
function Assert-ManagedBaseline {
    ...
    if ($Provider -eq 'lmstudio' -and $adapterOkay) { $adapterOkay = [bool]$adapterRows[0].details.expected -and [bool]$adapterRows[0].details.running }
    if ($Provider -eq 'ollama' -and $adapterOkay) { $adapterOkay = -not [bool]$adapterRows[0].details.expected }
    $gateway = Get-ListenerSnapshot -Port (Get-GatewayPort)
    $providerListener = Get-ListenerSnapshot -Port (Get-ProviderPort)
    $ok = ([string]$status.state.mode -eq 'managed') -and ([string]$providerDoc.selectedProvider -eq $Provider) -and $adapterOkay -and [bool]$gateway.listening -and [bool]$providerListener.listening
    Add-Step "assert-managed-$Label" $(if ($ok) { 'PASS' } else { 'FAIL' }) ([ordered]@{ mode=[string]$status.state.mode; selectedProvider=[string]$providerDoc.selectedProvider; adapter=$(if ($adapterRows.Count -eq 1) { $adapterRows[0] } else { $null }); gateway=$gateway; providerListener=$providerListener })
    if (-not $ok) { throw "Managed baseline failed at $Label." }
}

function Install-ReleasedCogentNexus {
    if (Test-Path $Cnx) { throw "-InstallRelease requires no existing cnx.cmd: $Cnx" }
    $api = "https://api.github.com/repos/funggier/cogentnexus/releases/tags/$ReleaseTag"
    $headers = @{ 'User-Agent'='CogentNexus-Recovery-Reality-v2'; 'Accept'='application/vnd.github+json' }
    ...
    if ($ExpectedReleaseCommit -and [string]$release.target_commitish -ne $ExpectedReleaseCommit) { throw "Release target mismatch." }
    $name = "cogentnexus-$ReleaseTag"
    $zipName = "$name.zip"
    $assets = @($release.assets)
    $zipAsset = @($assets | Where-Object { $_.name -eq $zipName })
    $sumAsset = @($assets | Where-Object { $_.name -eq 'SHA256SUMS.txt' })
    ...
    Invoke-CapturedProcess 'release-consumer-install' 'powershell.exe' @('-NoProfile','-ExecutionPolicy','Bypass','-File',$installer,'-Provider',$Provider) @(0) 1200 | Out-Null
...
function Confirm-DisruptiveSuite {
    ...
    Write-Host 'Exact listener PIDs will be validated before force-kill. Process trees are never killed.'
...
function Invoke-ProviderCrashScenario {
    ...
    $recovery = Get-JsonCommand 'recovery-after-provider-hard-crash' @('check','recovery','--json') @(0,1)
    ...
    if ([bool]$incidentRows[0].details.circuitOpen) { throw 'Provider recovery circuit open after one injected crash.' }
    Assert-ManagedBaseline 'provider-crash-after'
    Add-Step 'scenario-provider-crash' 'PASS' ([ordered]@{ before=$before; after=$after; recoveryIncident=$incidentRows[0] })
...
function Invoke-OperatorStopScenario {
    ...
    Invoke-CapturedProcess 'intentional-cnx-stop' $Cnx @('stop') @(0) 600 | Out-Null
...
function Invoke-BestEffortReconcile {
    ...
    try {
        Invoke-CapturedProcess 'cleanup-reconcile-start' $Cnx @('start','--provider',$Provider) @(0) 900 | Out-Null
        Assert-ManagedBaseline 'cleanup'
        Add-Step 'cleanup-reconcile' 'PASS' ([ordered]@{ provider=$Provider })
    } catch {
        Add-Step 'cleanup-reconcile' 'FAIL' ([ordered]@{ error=$_.Exception.Message })
    }
}

Set-Content -Path $LogPath -Value "CogentNexus v0.9.3 Recovery Reality Windows Harness v2`r`n" -Encoding UTF8
Save-Evidence
...
try {
    ...
    foreach ($name in $ExpandedScenarios) {
        switch ($name) {
            'baseline' { Assert-ManagedBaseline 'baseline'; Add-Step 'scenario-baseline' 'PASS' ([ordered]@{ provider=$Provider }) }
    ...
    Clear-ActiveOperation
    Save-Evidence
    Write-Evidence 'COGENTNEXUS v0.9.3 RECOVERY REALITY SUITE v2: PASS'
