...
7. If a supported read method exists, capture only the minimum required redacted fields for the CNX-354 objective:
   - Gateway PID/process identity if exposed;
   - loaded CogentNexus module path/identity if exposed;
   - plugin-manager owner/registration identity if exposed;
   - `before_agent_run` handler membership/priority/owner if exposed;
   - registration invocation/completion state if exposed.
...
9. Preserve raw evidence and publish:
...
- exact branch;
- authenticated client boundary and non-secret scope evidence;
- exact supported read-only methods examined;
- process-local evidence, or a precise statement of why it remains unavailable;
- no-secret accounting;
- hard-fence accounting;
- changed-path list;
- ancestry and remote read-back verification;
