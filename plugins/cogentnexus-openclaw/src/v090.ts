...
function suppressSessionWorkflowCompletions(workspace: string, sessionKey: string, reason: string) {
  const root = resolve(workspace, ".cogentnexus-openclaw", "workflows"), tags: string[] = [];
...
function cancelBoundWorkflows(workspace: string, workflowIds: string[], reason: string, cfg: Cfg) {
  const runtime = resolve(workspace, "skills", "cogentnexus-openclaw", "scripts", "workflow.py");
...
export function patchTicketStore() {
  ...
  TicketStore.prototype.accept = function(input: Parameters<TicketStore["accept"]>[0]) {
    ...
    try {
      ...
      if (authority.state !== "active") throw new Error(`CogentNexus-OpenClaw session is ${authority.state}: ${input.ownerSessionKey}`);
    } finally { db.close(); }
...
function resetCompletions(workspace: string) {
  const root = resolve(workspace, ".cogentnexus-openclaw", "workflows");
...
function isInternalControlText(text: string) {
  return /#cogent-direct\b|\[CogentNexus-OpenClaw (?:Delivery|Continuation|Internal|Direct Recovery):|\[CogentNexus-OpenClaw Internal /iu.test(text);
}
...
function recoveryPrompt(recovery: Recovery, context: string) {
  ...
  return [
    "[CogentNexus-OpenClaw Internal Direct Recovery]",
...
function compatibilityPrompt(input: Turn, context: string) {
  return [
    "[CogentNexus-OpenClaw Internal Delivery Worker]",
...
function hostDeliveryScript(workspace: string) {
  return resolve(workspace, "skills", "cogentnexus-openclaw", "scripts", "host_delivery.py");
}
...
function markResponseReady(path: string, recovery: Recovery, runId: string, text: string) {
  ...
  try {
    ...
    const idempotencyKey = `cnxclaw-direct-result:${recovery.ticket_id}:g${recovery.owner_generation}`;
...
function cnxOwnedTask(task: any) {
  return task?.label === "plugin:cogentnexus-openclaw"
...
export async function launchRecovery(api: any, path: string, workspace: string, recovery: Recovery, cfg: Cfg) {
  ...
  const planned = `cnxclaw-direct-${recovery.ticket_id.replace(/[^A-Za-z0-9_-]/g, "-").slice(0, 48)}-${attempt}-g${recovery.owner_generation}`;
    ...
    start: async (ctx: any) => {
      ...
      const tick = async () => {
        ...
        try {
          ...
          api.logger.warn(`CogentNexus-OpenClaw Direct recovery scan failed: ${error instanceof Error ? error.message : String(error)}`);
...
export async function executeCompatibilityWake(api: any, cfg: Cfg, input: Turn) {
  ...
  try {
    ...
    api.logger.warn(`CogentNexus-OpenClaw compatibility wake failed for ${input.tag}: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  } finally {
...
function activeWorkflowForSession(workspace: string, requestHash: string, ownerSessionKey: string) {
  const base = resolve(workspace, ".cogentnexus-openclaw", "workflows");
  ...
  for (const entry of readdirSync(base, { withFileTypes: true })) {
    ...
    try {
      ...
      for (const step of manifest?.steps ?? []) for (const output of step?.outputs ?? []) {
        if (typeof output !== "string" || output.replace(/\\/g, "/").startsWith(".cogentnexus-openclaw/")) continue;
    ...
    start: async (ctx: any) => {
      ...
      const tick = async () => {
        ...
        try {
          ...
          for (const item of reconcileTicketWorkflows({ workspaceDir: workspace, store, config: cfg as any })) api.logger.info?.(`CogentNexus-OpenClaw Ticket ${item.ticketId} workflow action ${item.action}`);
          const dispatched = safeDispatchTicketWorkflows({ workspaceDir: workspace, store, config: cfg });
          for (const item of dispatched.deferred) api.logger.info?.(`CogentNexus-OpenClaw Ticket ${item.ticketId} deferred: ${item.reason}`);
  ...
  baseEntry.register = (api: any) => {
    ...
    api.on?.("agent_end", async (event: any, ctx: any) => {
      ...
      for (const tag of [...cancelled.outboxTags, ...completionTags]) {
        ...
        catch (error) { api.logger.warn?.(`CogentNexus-OpenClaw cancellation unschedule failed for ${tag}: ${error instanceof Error ? error.message : String(error)}`); }
      }
      const results = cancelBoundWorkflows(workspace, cancelled.workflowIds, reason, cfg);
      for (const result of results) if (!result.ok) api.logger.warn?.(`CogentNexus-OpenClaw workflow cancellation failed for ${result.workflowId}: ${result.error}`);
    ...
    api.on?.("session_end", async (event: any, ctx: any) => {
      ...
      for (const result of results) if (!result.ok) api.logger.warn?.(`CogentNexus-OpenClaw session reset workflow cancellation failed for ${result.workflowId}: ${result.error}`);
      await cancelTrackedSynthetic(api, sessionKey, ctx.config ?? {});
      api.logger.info?.(`CogentNexus-OpenClaw session reset barrier completed for ${sessionKey}: generation=${reset.generation} tickets=${reset.cancelled.length} assistantSuppressed=${reset.assistantSuppressed}`);
    ...
    api.on?.("session_end", async (event: any, ctx: any) => {
      ...
      for (const result of results) if (!result.ok) api.logger.warn?.(`CogentNexus-OpenClaw session deletion workflow cancellation failed for ${result.workflowId}: ${result.error}`);
      await cancelTrackedSynthetic(api, sessionKey, ctx.config ?? {});
      finalizeSessionDeletion(path, sessionKey, reason);
      api.logger.info?.(`CogentNexus-OpenClaw session deletion barrier completed for ${sessionKey}: generation=${deletion.generation} tickets=${deletion.cancelled.length} assistantSuppressed=${deletion.assistantSuppressed}`);
