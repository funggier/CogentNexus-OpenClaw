#!/usr/bin/env python3
"""Read-only CogentNexus-OpenClaw preflight/check engine.
...
def check_cogentnexus(root: Path) -> list[dict[str, Any]]:
    ...
    required = ["host_control_v091.py", "host_v091.py", "lifecycle_v091.py", "provider.py", "checks.py", "cnxclaw.py"]
    missing = [name for name in required if not (here / name).is_file()]
    results = [
        item("CogentNexus-OpenClaw installation", "FAIL" if missing else "PASS", "Required CogentNexus-OpenClaw files are missing" if missing else "Core/provider/check scripts are present", missing=missing)
    ]
...
def resolve_provider(root: Path, override: str | None) -> tuple[str | None, list[dict[str, Any]]]:
    ...
    return None, [item("Provider selection", "FAIL", "Multiple providers are installed but no provider has been selected", installed=installed, suggested=[f"cnxclaw.cmd start --provider {name}" for name in installed])]
...
def component_check(root: Path, component: str, provider_override: str | None = None) -> dict[str, Any]:
    root = root.resolve()
    mapping = {
        "cogentnexus-openclaw": lambda: check_cogentnexus(root), "config": lambda: check_config(root),
...
def render(report: dict[str, Any]) -> str:
    lines = ["CogentNexus-OpenClaw System Check", "=" * 58]
