...
def test_canonical_install_docs_separate_install_prerequisites_from_runtime_readiness():
    for path in DOCS:
        ...
        end_marker = "## Exact release/source-tree install" if path.name == "INSTALL.md" else "## ติดตั้งจาก exact release/source tree"
...
def test_canonical_install_docs_do_not_claim_installer_owns_provider_preflight():
    for path in DOCS:
        ...
        install_section = text.split("## exact release/source-tree install", 1)[1].split("## what the installer does", 1)[0] if path.name == "INSTALL.md" else text.split("## ติดตั้งจาก exact release/source tree", 1)[1].split("## สิ่งที่ installer ทำ", 1)[0]
