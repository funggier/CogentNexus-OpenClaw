#!/usr/bin/env python3
"""CogentNexus-OpenClaw Host control wrapper for OpenClaw compatibility policy.

OpenClaw 2026.7.1-2 can classify a diagnostic stalled-run abort as a generic
user abort after losing the original stuck_recovery reason. While CogentNexus-OpenClaw
owns runtime lifecycle, keep that native watchdog outside the normal CNXCLAW run
horizon. The user's previous OpenClaw value is snapshotted and restored on
`cnxclaw disable`; user changes made while managed are never overwritten.
"""
...
def apply_watchdog_compat(root: Path) -> dict[str, Any]:
    ...
    if existing:
        try:
            ...
            raise RuntimeError(f"invalid CogentNexus-OpenClaw watchdog snapshot: {error}") from error
    else:
    ...
    if existing and snapshot.get("applied") is True and (not present or current != MANAGED_WATCHDOG_ABORT_MS):
        ...
        raise RuntimeError(
            "OpenClaw diagnostics.stuckSessionAbortMs changed while CogentNexus-OpenClaw was managed; "
...
def main() -> int:
    ...
    if command == "disable":
        ...
        # the CNXCLAW compatibility fence back immediately rather than leaving a
        # half-disabled managed runtime exposed to the ambiguous 2026.7.1-2
        # stuck-session abort path.
        try:
            restore_watchdog_compat(root)
        except Exception as error:
            print(json.dumps({"result": "error", "error": f"CogentNexus-OpenClaw watchdog compatibility restore failed: {error}"}, ensure_ascii=False, indent=2))
            return 1
        code = delegate(argv)
        if code != 0:
            rollback_error = None
            ...
            if rollback_error:
                print(json.dumps({
                    "result": "error",
                    "error": "CogentNexus-OpenClaw disable failed and watchdog compatibility rollback also failed",
    ...
    try:
        ...
        print(json.dumps({"result": "error", "error": f"CogentNexus-OpenClaw watchdog compatibility failed: {error}"}, ensure_ascii=False, indent=2))
        return 1
