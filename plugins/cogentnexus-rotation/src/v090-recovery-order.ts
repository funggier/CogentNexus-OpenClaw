...
export function predecessorRecovery(path:string,input:{ticketId:string;ownerSessionKey:string;ownerGeneration:number}) {
  ...
  try {
    const current=db.prepare("SELECT created_at FROM tickets WHERE ticket_id=? AND owner_session_key=?").get(input.ticketId,input.ownerSessionKey) as any;
    if(!current)return undefined;
    return db.prepare(`SELECT older.ticket_id,older.created_at,recovery.state,recovery.next_attempt_at
      FROM tickets older JOIN cnx_direct_recovery recovery ON recovery.ticket_id=older.ticket_id
      WHERE older.owner_session_key=? AND older.ticket_id<>? AND older.status='accepted'
        ...
        AND (older.created_at<? OR (older.created_at=? AND older.ticket_id<?))
      ORDER BY older.created_at,older.ticket_id LIMIT 1`)
      .get(input.ownerSessionKey,input.ticketId,input.ownerGeneration,current.created_at,current.created_at,input.ticketId) as any;
  } finally {db.close();}
}
...
export function directRecoveryIdentity(path:string,ticketId:string,ownerGeneration:number) {
  ...
  try {
    ...
    return {authorized:true,ticketId:row.ticket_id,ownerSessionKey:row.owner_session_key,ownerGeneration,createdAt:row.created_at,recoveryState:row.recovery_state};
  } finally {db.close();}
}
...
export function queueBehindOlderRecovery(path:string,input:{sessionKey:string;runId:string;reason?:string}) {
  ...
  try {
    ...
    const current=db.prepare(`SELECT t.ticket_id,t.created_at,t.status,t.workflow_eligible,t.workflow_id,s.state AS session_state,s.generation
    ...
    const predecessor=db.prepare(`SELECT older.ticket_id,older.created_at,recovery.state,recovery.next_attempt_at
      FROM tickets older JOIN cnx_direct_recovery recovery ON recovery.ticket_id=older.ticket_id
      WHERE older.owner_session_key=? AND older.ticket_id<>? AND older.status='accepted'
        AND older.workflow_eligible=0 AND older.workflow_id IS NULL
        AND recovery.owner_generation=? AND recovery.state IN ('pending','running','awaiting_delivery')
        AND (older.created_at<? OR (older.created_at=? AND older.ticket_id<?))
      ORDER BY older.created_at,older.ticket_id LIMIT 1`)
      .get(input.sessionKey,current.ticket_id,current.generation,current.created_at,current.created_at,current.ticket_id) as any;
    ...
    db.prepare("INSERT INTO ticket_events(ticket_id,event_type,payload_json,created_at) VALUES (?,?,?,?)")
      .run(current.ticket_id,"direct_recovery_serialized",JSON.stringify({ownerSessionKey:input.sessionKey,ownerGeneration:Number(current.generation),predecessorTicketId:predecessor.ticket_id,predecessorState:predecessor.state,reason}),now);
    db.exec("COMMIT");
    return {ticketId:String(current.ticket_id),ownerGeneration:Number(current.generation),predecessorTicketId:String(predecessor.ticket_id),predecessorState:String(predecessor.state)};
...
export function installRecoveryOrderAdmission(api:any,config:RecoveryOrderConfig={}) {
  api.on?.("before_agent_run",async(_event:any,ctx:any)=>{
    ...
    api.logger.info?.(`CogentNexus serialized Ticket ${queued.ticketId} behind ${queued.predecessorTicketId} (${queued.predecessorState}) in ${sessionKey}`);
    return {
      outcome:"block",
      reason:"CogentNexus committed this request and queued it behind older Direct Recovery in the same session",
      category:"cogentnexus_recovery_order",
      metadata:queued,
    };
