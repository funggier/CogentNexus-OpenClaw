 ...
 * CogentNexus owns recovery for this exact session:
 ...
 * CogentNexus Direct Recovery is already running, creating a second recovery
 ...
 * only the duplicate native restart system turn; CogentNexus keeps the durable
...
export function installV099NativeRestartOwnershipFence(api: any, config: Config) {
   ...
   *   2) positive durable same-session CogentNexus Direct Recovery authority.
   *
  ...
  api.on(
    ...
    (event: any, ctx: any) => {
      ...
      api.logger.info?.(
        `CogentNexus v0.9.9 suppressed OpenClaw native restart recovery ` +
      ...
      return {
        outcome: "block",
        reason:
          "duplicate OpenClaw native restart continuation is already owned by CogentNexus Direct Recovery",
