...
class InteractiveDelegationTests(unittest.TestCase):
    ...
    def test_interactive_non_y_is_forwarded_and_cancels(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            fake_host = tmp_path / "fake_host.py"
            fake_host.write_text(
            ...
            runner.write_text(
                textwrap.dedent(
                    ...
                    import cnxclaw
                    cnxclaw.HOST_CONTROL = Path({str(fake_host)!r})
                    raise SystemExit(cnxclaw.main(['--root', {str(tmp_path)!r}, 'uninstall']))
                    """
                ),
                encoding="utf-8",
            )
            ...
            proc.stdout.close()
            proc.stderr.close()
            self.assertTrue(prompt_before_input)
            self.assertEqual(proc.returncode, 1)
            self.assertEqual(stderr, "")
            self.assertIn("ACK:n", observed)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runner.write_text(
                textwrap.dedent(
                    ...
                    import cnxclaw
                    cnxclaw.HOST_CONTROL = Path({str(fake_host)!r})
                    raise SystemExit(cnxclaw.main(['--root', {str(tmp_path)!r}, 'reset']))
                    """
                ),
                encoding="utf-8",
            )
