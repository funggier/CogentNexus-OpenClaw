...
ใช้ `scripts/clean-reinstall.ps1` เมื่ออยากเอา CogentNexus-OpenClaw ที่ติดตั้งอยู่ใน workspace ปัจจุบันออกให้สะอาด แล้วติดตั้ง v0.9.5 ใหม่จาก candidate/source package ปัจจุบัน
...
ค่าเริ่มต้นของสคริปต์จึง **backup ก่อนลบเสมอ** และเก็บ backup ไว้นอก workspace รวมถึงอยู่นอก active CogentNexus-OpenClaw application-data tree ที่:
...
7. เรียก `scripts/install.ps1` จาก candidate/source package ปัจจุบัน
8. ตรวจ CNXCLAW status, Gateway และ plugin inventory หลังติดตั้ง

ถ้าคืน PASSTHROUGH ไม่สำเร็จ หรือ backup root ที่กำหนดอยู่ภายใน active application-data tree สคริปต์จะหยุดก่อนการลบแบบ destructive
