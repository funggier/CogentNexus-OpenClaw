  ...
  const run = () => {
    ...
    void (async () => {
      try {
        ...
        logger.warn?.(`CogentNexus-OpenClaw event worker failed: ${error instanceof Error ? error.message : String(error)}`);
      } finally {
        active = false;
      }
    })();
...
function createEventDrivenWorkflowCompletion(api: any, config: any, startupFences:StartupFences) {
  ...
  const service: any = {
    ...
    start: async (ctx: any) => {
      await awaitStartupFences(startupFences,ctx);
      const workspaceDir = resolve(config.workspaceDir ?? ctx?.config?.agents?.defaults?.workspace ?? process.cwd());
      const workflowsDir = resolve(workspaceDir, ".cogentnexus-openclaw", "workflows");
      ...
      try {
        ...
        watcher.on("error", (error) => api.logger.warn(`CogentNexus-OpenClaw workflow filesystem watcher degraded: ${error.message}`));
      } catch (error) {
        api.logger.warn(`CogentNexus-OpenClaw workflow filesystem watcher unavailable: ${error instanceof Error ? error.message : String(error)}`);
      }
    ...
    start: async (ctx: any) => {
      ...
      const tick = async (forceDeep = false) => {
        ...
        try {
          ...
          api.logger.warn(`CogentNexus-OpenClaw adaptive Host reconciliation failed: ${error instanceof Error ? error.message : String(error)}`);
        } finally {
          active = false;
        }
      };
  ...
  entry.register = (api: any) => {
    ...
    if (rawRegister) {
      proxy.registerService = (service: any) => {
        const startupFences:StartupFences = [
          service?.[PRE_RUNTIME_FENCE] as StartupFence,
          service?.[STARTUP_RECOVERY_FENCE] as StartupFence,
        ];
    ...
    for (const eventName of ["agent_end", "message_sent", "session_end", "after_compaction", "reply_dispatch", "subagent_ended"]) {
