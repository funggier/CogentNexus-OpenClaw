...
```text
python skills/cogentnexus-openclaw/scripts/runtime.py scheduler detect
python skills/cogentnexus-openclaw/scripts/runtime.py scheduler render --backend systemd
```
...
- PASSTHROUGH performs no CogentNexus-OpenClaw recovery;
...
Windows child commands use `CREATE_NO_WINDOW`; POSIX detached children use session/background semantics appropriate to the adapter. Diagnostics may go to platform logs, but durable runtime/workflow evidence remains under `.cogentnexus-openclaw`.
