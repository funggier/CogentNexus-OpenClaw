...
This module intentionally owns only provider discovery/lifecycle/readiness. It does
not own Ticket, recovery, delivery, or workflow semantics.
"""
...
def find_ollama_cli() -> str | None:
    ...
    return _existing([
        shutil.which("ollama"),
        str(Path(local) / "Programs" / "Ollama" / "ollama.exe") if local else None,
    ])
...
def find_lms_cli() -> str | None:
    ...
    candidates = [
        shutil.which("lms"),
        shutil.which("lms.exe"),
        shutil.which("lms.cmd"),
    ...
    if local:
        candidates.extend([
            str(Path(local) / "Programs" / "LM Studio" / "bin" / "lms.exe"),
            str(Path(local) / "Programs" / "LM Studio" / "LM Studio.exe"),
        ])
...
def find_lmstudio_gui() -> str | None:
    ...
    if local:
        candidates.extend([
            str(Path(local) / "Programs" / "LM Studio" / "LM Studio.exe"),
            str(Path(local) / "LM Studio" / "LM Studio.exe"),
        ])
    if program_files:
...
def _http_json(url: str, timeout: float = 5.0) -> tuple[bool, Any]:
...
def probe(name: str, timeout: float = 5.0) -> dict[str, Any]:
    provider = normalize_provider(name)
    info = detect(provider)
    if provider == "ollama":
        ok, evidence = _http_json(f"{info['endpoint']}/api/tags", timeout)
    else:
        ok, evidence = _http_json(f"{info['endpoint']}/api/v1/models", timeout)
        if not ok:
            fallback_ok, fallback = _http_json(f"{info['endpoint']}/v1/models", timeout)
            if fallback_ok:
    ...
    models = _extract_model_ids(provider, body)
    return {
        **info,
        "reachable": bool(ok),
        "healthy": bool(ok),
...
def _start_lmstudio() -> dict[str, Any]:
    ...
    if not cli:
        return {
            "ok": False,
            "error": "LM Studio is installed but the 'lms' CLI is unavailable; enable/install the LM Studio CLI first",
        }
...
def start(name: str, timeout: float = 30.0) -> dict[str, Any]:
    provider = normalize_provider(name)
    before = probe(provider, timeout=min(timeout, 5.0))
    if before["healthy"]:
        return {"ok": True, "skipped": True, "reason": "already healthy", "before": before, "after": before}
    if not before["installed"]:
        return {"ok": False, "error": f"provider '{provider}' is not installed", "before": before}
    action = _start_ollama() if provider == "ollama" else _start_lmstudio()
    if not action.get("ok"):
        return {"ok": False, "action": action, "before": before}
    deadline = time.monotonic() + max(1.0, timeout)
    after = probe(provider, timeout=3.0)
...
def _stop_ollama() -> dict[str, Any]:
    if os.name == "nt":
        ...
        for image in ("ollama app.exe", "ollama.exe"):
            attempts.append(run([taskkill, "/IM", image, "/T", "/F"], timeout=30))
...
def stop(name: str, timeout: float = 30.0) -> dict[str, Any]:
    provider = normalize_provider(name)
    before = probe(provider, timeout=3.0)
    if not before["healthy"]:
        return {"ok": True, "skipped": True, "reason": "already stopped or unreachable", "before": before, "after": before}
    action = _stop_ollama() if provider == "ollama" else _stop_lmstudio()
    deadline = time.monotonic() + max(1.0, timeout)
    after = probe(provider, timeout=2.0)
...
def openclaw_model_status() -> dict[str, Any]:
    ...
    def find_default(value: Any) -> str | None:
        if isinstance(value, dict):
            preferred = ("defaultModel", "default", "primary", "model")
            for key in preferred:
                item = value.get(key)
                if isinstance(item, str) and "/" in item:
                    return item
                if isinstance(item, dict):
                    nested = item.get("primary") or item.get("default")
                    if isinstance(nested, str) and "/" in nested:
                        return nested
            for item in value.values():
                found = find_default(item)
                if found:
                    return found
