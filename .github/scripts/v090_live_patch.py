...
def patch_host() -> None:
    p = Path("skills/cogentnexus/scripts/host.py")
    s = p.read_text(encoding="utf-8")

    old = '''def gateway_status(timeout: int = 30) -> dict[str, Any]:
    try:
...
'''
    new = '''def gateway_status(timeout: int = 30) -> dict[str, Any]:
    try:
        result = run([openclaw_executable(), "gateway", "status"], timeout=timeout)
        stdout = result.stdout.strip()
        stderr = result.stderr.strip()
...
'''
    if old not in s:
        raise SystemExit("gateway_status block not found")
...
def reconcile_terminal_fences(root: Path) -> dict[str, int]:
    ...
    try:
        ...
        db.close()
'''
    if "def reconcile_terminal_fences(" not in s:
        if marker not in s:
            raise SystemExit("cancel_ticket marker not found")
        s = s.replace(marker, helper + marker, 1)

    old_terminal = '''        if row["status"] in TERMINAL_TICKETS:
            return {"ticketId": ticket_id, "status": row["status"], "changed": False}
'''
    new_terminal = '''        if row["status"] in TERMINAL_TICKETS:
            if row["status"] == "cancelled":
                reconcile_terminal_fences(root)
            return {"ticketId": ticket_id, "status": row["status"], "changed": False}
'''
    if old_terminal not in s:
        raise SystemExit("terminal cancel block not found")
    s = s.replace(old_terminal, new_terminal, 1)

    old_status = "status='cancelled',worker_id=NULL,lease_token=NULL,lease_expires_at=NULL,heartbeat_at=NULL,failure_class='interrupted',failure_message=?,updated_at=?"
    new_status = "status='cancelled',worker_id=NULL,lease_token=NULL,lease_expires_at=NULL,heartbeat_at=NULL,failure_class=NULL,failure_message=?,updated_at=?"
    if old_status not in s:
        raise SystemExit("cancel status SQL not found")
    s = s.replace(old_status, new_status, 1)

    old_outbox = '''            if table_exists(db, "ticket_outbox"):
...
'''
    new_outbox = '''            if table_exists(db, "ticket_outbox"):
...
'''
    if old_outbox not in s:
        raise SystemExit("host cancelled-outbox block not found")
    s = s.replace(old_outbox, new_outbox, 1)

    old_enable = '''def enable(root: Path) -> dict[str, Any]:
    initialize(root)
    workspace = root.parent
'''
    new_enable = '''def enable(root: Path) -> dict[str, Any]:
    initialize(root)
    terminal_fences = reconcile_terminal_fences(root)
    workspace = root.parent
'''
    if old_enable not in s:
        raise SystemExit("enable block not found")
    s = s.replace(old_enable, new_enable, 1)

    old_enable_return = '''        "sessionBootstrap": session_bootstrap,
        "recoveredTickets": recovered,
'''
    new_enable_return = '''        "sessionBootstrap": session_bootstrap,
        "terminalFences": terminal_fences,
        "recoveredTickets": recovered,
'''
    if old_enable_return not in s:
        raise SystemExit("enable return block not found")
    s = s.replace(old_enable_return, new_enable_return, 1)

    old_start = '''def start_managed(root: Path, provider: bool = True) -> dict[str, Any]:
    initialize(root)
    prior = load_state(root)
'''
    new_start = '''def start_managed(root: Path, provider: bool = True) -> dict[str, Any]:
    initialize(root)
    terminal_fences = reconcile_terminal_fences(root)
    prior = load_state(root)
'''
    if old_start not in s:
        raise SystemExit("start_managed block not found")
    s = s.replace(old_start, new_start, 1)

    old_start_return = '    return {"state": state, "lifecycle": parse_json_output(result.stdout), "sessionBootstrap": session_bootstrap, "recoveredTickets": recovered}\n'
    new_start_return = '    return {"state": state, "lifecycle": parse_json_output(result.stdout), "sessionBootstrap": session_bootstrap, "terminalFences": terminal_fences, "recoveredTickets": recovered}\n'
    if old_start_return not in s:
        raise SystemExit("start return block not found")
    s = s.replace(old_start_return, new_start_return, 1)
...
def patch_v090() -> None:
    ...
    start = s.index("export function cancelSessionTickets(")
    end = s.index("\nfunction suppressWorkflowCompletion", start)
    replacement = '''export function cancelSessionByKey(path:string,input:{sessionKey:string;message?:string;now?:Date}){
...
'''
    ...
    anchor = "function cancelBoundWorkflows(workspace:string,workflowIds:string[],reason:string,cfg:Cfg){"
    helper = '''function suppressSessionWorkflowCompletions(workspace:string,sessionKey:string,reason:string){
...
'''
    if "function suppressSessionWorkflowCompletions(" not in s:
        if anchor not in s:
            raise SystemExit("workflow cancellation anchor missing")
        s = s.replace(anchor, helper + anchor, 1)

    old_vars = "const path=dbPath(cfg,workspace),db=openDb(path),stamp=now();let reopened=0,outboxReset=0,cancelledLegacy=0;"
    new_vars = "const path=dbPath(cfg,workspace),db=openDb(path),stamp=now();let reopened=0,outboxReset=0,cancelledLegacy=0,cancelledOutboxSuppressed=0,terminalRecoverySuppressed=0;"
    if old_vars not in s:
        raise SystemExit("prepare vars marker missing")
    s = s.replace(old_vars, new_vars, 1)

    old_begin = 'try{db.exec("BEGIN IMMEDIATE");const rows=db.prepare('
    new_begin = '''try{db.exec("BEGIN IMMEDIATE");
    ...
    if old_begin not in s:
        raise SystemExit("prepare begin marker missing")
    s = s.replace(old_begin, new_begin, 1)

    old_return = "return{databasePath:path,reopened,cancelledLegacy,outboxReset,workflowDeliveryReset:resetCompletions(workspace)};"
    new_return = "return{databasePath:path,reopened,cancelledLegacy,cancelledOutboxSuppressed,terminalRecoverySuppressed,outboxReset,workflowDeliveryReset:resetCompletions(workspace)};"
    if old_return not in s:
        raise SystemExit("prepare return marker missing")
    s = s.replace(old_return, new_return, 1)

    wrap_start = s.index("function wrap(){")
    wrap_end = s.index("\nwrap();", wrap_start)
    new_wrap = '''function wrap(){
  ...
  baseEntry.register=(api:any)=>{
    ...
    s = s[:wrap_start] + new_wrap + s[wrap_end:]
    p.write_text(s, encoding="utf-8")
...
def patch_host_tests() -> None:
    ...
    old_schema = "        CREATE TABLE ticket_outbox(outbox_id INTEGER PRIMARY KEY AUTOINCREMENT,ticket_id TEXT UNIQUE,owner_session_key TEXT,terminal_status TEXT,payload_json TEXT,delivery_status TEXT,created_at TEXT);\n"
    new_schema = old_schema + "        CREATE TABLE cnx_direct_recovery(ticket_id TEXT PRIMARY KEY,state TEXT,active_run_id TEXT,next_attempt_at TEXT,last_error TEXT,updated_at TEXT);\n"
    if old_schema not in s:
        raise SystemExit("host test schema marker missing")
    s = s.replace(old_schema, new_schema, 1)

    start = s.index("    def test_ticket_cancel_is_terminal_and_enqueues_outbox(self):")
    end = s.index("\n    def test_passthrough_supervisor_does_nothing", start)
    replacement = '''    def test_ticket_cancel_is_terminal_silent_and_suppresses_recovery(self):
...
def patch_v090_tests() -> None:
    p = Path("plugins/cogentnexus-rotation/src/v090.test.ts")
    s = p.read_text(encoding="utf-8")
    if "  cancelSessionByKey,\n" not in s:
        s = s.replace("  cancelSessionTickets,\n", "  cancelSessionByKey,\n  cancelSessionTickets,\n", 1)
    anchor = '  it("keeps a generic interruption recoverable instead of guessing user intent", () => {'
    if "UI Stop suppresses pending synthetic delivery" not in s:
        addition = '''  it("UI Stop suppresses pending synthetic delivery even when no non-terminal Ticket remains", () => {
...
'''
        if anchor not in s:
            raise SystemExit("v090 test anchor missing")
