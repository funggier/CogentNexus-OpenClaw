...
When a workflow belongs to an OpenClaw conversation, call `workflow.py bind-owner <task-id> --session-key <trusted-key>` after initialization. A terminal transition commits `completion.json` as a durable outbox. The CogentNexus-OpenClaw plugin polls pending outboxes, mirrors the terminal result into a managed TaskFlow, schedules one idempotently tagged owner turn, and marks delivery only after scheduling succeeds.
...
State and ledger evidence live under `.cogentnexus-openclaw/workflows/<task-id>`. Manifest integrity is checked before transitions. A controller PID fences duplicate controllers, while step runner PIDs fence duplicate execution.
