...
function cnxOwnedNativeTask(task: NativeTaskView) {
  return task.label === CNXCLAW_PLUGIN_LABEL
...
export function shouldFenceNativeCnxTask(
  ...
  if (/^\[CogentNexus-OpenClaw Delivery: ticket:\d+\]/u.test(title)
      && /reached terminal status (?:failed|cancelled)\./u.test(title)) return true;
  if (/^\[CogentNexus-OpenClaw Delivery: workflow:/u.test(title)
  ...
  for (const row of rows) {
    ...
    if (taskRuns?.bindSession) {
      try {
        ...
        rowFailed = true;
        api.logger.warn?.(`CogentNexus-OpenClaw stale synthetic fence could not cancel ${row.childSessionKey}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    if (!rowFailed) {
      try { await api.runtime?.subagent?.deleteSession?.({ sessionKey:row.childSessionKey, deleteTranscript:true }); }
        ...
        if (!/not found|unknown session|does not exist/iu.test(message)) {
          rowFailed = true;
          api.logger.warn?.(`CogentNexus-OpenClaw stale synthetic fence could not delete ${row.childSessionKey}: ${message}`);
        }
      }
    }
    if (rowFailed) {
...
export async function reconcileOpenClawNativeTasks(
  ...
  for (const { sessionKey, agentId } of scopes) {
    ...
    try {
      ...
      api.logger.warn?.(`CogentNexus-OpenClaw native task fence could not bind ${sessionKey}: ${error instanceof Error ? error.message : String(error)}`);
      failed++;
      continue;
    }
    ...
    for (const task of tasks) {
      ...
      try {
        ...
        if (result?.cancelled) fenced++;
          ...
          failed++;
          api.logger.warn?.(`CogentNexus-OpenClaw native task fence did not cancel ${task.id}: ${result?.reason ?? "unknown reason"}`);
        }
      } catch (error) {
        failed++;
        api.logger.warn?.(`CogentNexus-OpenClaw native task fence failed for ${task.id}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
  return {
  ...
  entry.register = (api: any) => {
    ...
    api.on?.("subagent_spawned", (event: any, ctx: any) => {
      ...
      try {
        ...
        api.logger.warn?.(`CogentNexus-OpenClaw synthetic registry spawn failed for ${childSessionKey}: ${error instanceof Error ? error.message : String(error)}`);
      }
    ...
    api.on?.("subagent_ended", (event: any, ctx: any) => {
      ...
      try {
        ...
        api.logger.warn?.(`CogentNexus-OpenClaw synthetic registry settlement failed for ${childSessionKey}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }, { priority:2000, timeoutMs:5000 });
          ...
          try {
            ...
            if (current - lastOwnerReconcileAt >= 30_000) {
              ...
              if (owners.deleted > 0 || owners.failed > 0 || owners.workflowFailures > 0) {
                api.logger.info?.(`CogentNexus-OpenClaw owner reconciliation: checked=${owners.checked} deleted=${owners.deleted} failed=${owners.failed} workflowFailures=${owners.workflowFailures}`);
              }
            }
            const live = reconcileV090LiveState(databasePath);
            if (live.abortFailuresCancelled || live.abortOutboxSuppressed || live.failedOutboxSuppressed || live.terminalRecoverySuppressed) {
              api.logger.info?.(`CogentNexus-OpenClaw v0.9.0 live policy reconciliation: abortFailuresCancelled=${live.abortFailuresCancelled} abortOutboxSuppressed=${live.abortOutboxSuppressed} failedOutboxSuppressed=${live.failedOutboxSuppressed} terminalRecoverySuppressed=${live.terminalRecoverySuppressed}`);
            }
            const native = await reconcileOpenClawNativeTasks(api, ctx, databasePath);
            if (native.fenced > 0 || native.failed > 0 || native.syntheticFenced > 0 || native.syntheticFailed > 0) {
              api.logger.info?.(`CogentNexus-OpenClaw v0.9.0 native task fence: supported=${native.supported} scanned=${native.scanned} fenced=${native.fenced} failed=${native.failed} syntheticScanned=${native.syntheticScanned} syntheticFenced=${native.syntheticFenced} syntheticFailed=${native.syntheticFailed}`);
            }
            ...
            if (result.reopened > 0 || result.cancelledLegacy > 0) {
              api.logger.info?.(`CogentNexus-OpenClaw v0.9.0 reconciled Direct Tickets: reopened=${result.reopened} cancelledLegacy=${result.cancelledLegacy}`);
            }
          } catch (error) {
            api.logger.warn(`CogentNexus-OpenClaw v0.9.0 Host reconciliation failed: ${error instanceof Error ? error.message : String(error)}`);
          } finally {
            active = false;
          }
        };
