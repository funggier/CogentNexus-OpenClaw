...
export function createCompactionBoundaryApi(api:any) {
  ...
  if(originalOn)proxy.on=(name:string,handler:any,options?:any)=>{
    if(name==="after_compaction") {
      api.logger.info?.("CogentNexus-OpenClaw v0.9 suppressed legacy after_compaction synthetic continuation registration");
      return undefined;
    }
...
export function settleExistingContextHoldFromCompaction(input:{
  databasePath:string;sessionKey:string;tokenCount?:number;
  session?:{sessionId?:string;totalTokens?:number;totalTokensFresh?:boolean;contextTokens?:number}|null;now?:Date;
}) {
  ...
  try {
    ...
    const row=db.prepare(`SELECT session_key,owner_generation,ticket_id,state,context_window FROM cnx_context_maintenance
    ...
    if(observed===undefined||contextWindow===undefined) {
      db.prepare(`UPDATE cnx_context_maintenance SET session_id=COALESCE(?,session_id),last_tokens_after=COALESCE(?,last_tokens_after),
        last_action='native-compact-observed-unmeasured',updated_at=? WHERE session_key=? AND owner_generation=? AND state IN ('pending','degraded')`)
        .run(session.sessionId??null,observed??null,stamp,input.sessionKey,row.owner_generation);
      db.exec("COMMIT");return {found:true,settled:false,reason:"unmeasured",observedTokens:observed,contextWindow};
    }
    ...
    if(pressure.level!=="normal"){
      ...
      db.exec("COMMIT");return {found:true,settled:false,reason:"pressure-remains",observedTokens:observed,contextWindow};
    }
    const changed=db.prepare(`UPDATE cnx_context_maintenance SET state='done',session_id=COALESCE(?,session_id),
      context_window=?,projected_tokens=?,last_tokens_after=?,last_action='native-compact-satisfied',last_error=NULL,next_attempt_at=NULL,updated_at=?,completed_at=?
      WHERE session_key=? AND owner_generation=? AND state IN ('pending','degraded')`)
      .run(session.sessionId??null,contextWindow,observed,observed,stamp,stamp,input.sessionKey,row.owner_generation);
    if(changed.changes===1)db.prepare("INSERT INTO ticket_events(ticket_id,event_type,payload_json,created_at) VALUES (?,?,?,?)")
...
export function installPassiveCompactionObserver(api:any,config:CompactionBoundaryConfig={}) {
  api.on?.("after_compaction",async(event:any,ctx:any)=>{
    ...
    // Fast path is intentionally zero-RPC: the context row already carries the
    // window captured at admission and after_compaction.tokenCount is the native
    // post-compaction measurement. Most user Compact operations stop here.
    let result=settleExistingContextHoldFromCompaction({databasePath,sessionKey,tokenCount:event?.tokenCount,session:null});
    if(result.found&&result.reason==="unmeasured") {
      let session:any=null;
      try {const response=await api.runtime?.gateway?.request?.("sessions.describe",{key:sessionKey},{timeoutMs:5000});session=response?.session??null;} catch {}
      if(session)result=settleExistingContextHoldFromCompaction({databasePath,sessionKey,tokenCount:event?.tokenCount,session});
    }
    if(result.found)api.logger.info?.(`CogentNexus-OpenClaw passive compaction observation ${sessionKey}: settled=${result.settled} reason=${result.reason} tokens=${result.observedTokens??"?"}/${result.contextWindow??"?"}`);
