#!/usr/bin/env python3
"""CogentNexus-OpenClaw Host Controller.
...
def default_policy_template() -> Path:
    return SKILL / "templates" / "AGENTS.cogentnexus-openclaw.md"
...
def normalize_policy(text: str) -> str:
    ...
    if BEGIN in text or END in text:
        raise RuntimeError("managed policy source must not contain CogentNexus-OpenClaw block markers")
    return text.strip() + "\n"
...
def read_policy(root: Path) -> tuple[str, str]:
    ...
    if not template.exists():
        raise RuntimeError(f"CogentNexus-OpenClaw default policy template not found: {template}")
...
def merge_policy(existing: str, policy: str) -> str:
    ...
    if (start < 0) != (finish < 0) or (start >= 0 and finish < start):
        raise RuntimeError("AGENTS.md contains an incomplete CogentNexus-OpenClaw managed block")
    if start >= 0:
        finish += len(END)
...
def enable(root: Path) -> dict[str, Any]:
    ...
    recovered = promote_interrupted_direct(root, started, "CogentNexus-OpenClaw Host enabled after an interrupted OpenClaw runtime")
...
def start_managed(root: Path, provider: bool = True) -> dict[str, Any]:
    ...
    session_bootstrap = reconcile_default_session()
    recovered = promote_interrupted_direct(root, started, "Gateway resumed by CogentNexus-OpenClaw Host after interruption")
    runtime(root, "supervisor", "tick", "--execute-safe", timeout=180, check=False)
...
def restart_managed(root: Path) -> dict[str, Any]:
    ...
    result = runtime(root, "lifecycle", "restart", "--reason", "CogentNexus-OpenClaw Host requested restart", timeout=240, check=True)
    recovered = promote_interrupted_direct(root, started, "Gateway restarted by CogentNexus-OpenClaw Host")
    runtime(root, "supervisor", "tick", "--execute-safe", timeout=180, check=False)
...
def supervisor_tick(root: Path, execute_safe: bool) -> dict[str, Any]:
    ...
    if not before.get("healthy") and after.get("healthy"):
        recovered = promote_interrupted_direct(root, cutoff, "CogentNexus-OpenClaw Host confirmed Gateway recovery; prior direct turn was interrupted")
    return {
...
def command(args: argparse.Namespace) -> Any:
    ...
    if args.command in {"start", "stop", "restart"}:
        if load_state(root).get("mode") == "passthrough":
            raise RuntimeError(
                "CogentNexus-OpenClaw is disabled (PASSTHROUGH). Use 'cnxclaw enable' to enter MANAGED mode, "
                "or use 'cnxclaw gateway <start|stop|restart>' / native OpenClaw lifecycle commands "
                "without changing CogentNexus-OpenClaw mode."
            )
