...
async function snapshotCapsule(api:any,workspaceDir:string,databasePath:string,row:Maintenance,session:SessionDescription|null,reason:string) {
  ...
  const hash=createHash("sha256").update(row.session_key).digest("hex").slice(0,16),dir=resolve(workspaceDir,".cogentnexus-openclaw","context",hash);
...
async function maintain(api:any,row:Maintenance,config:ContextGuardConfig,workspaceDir:string,databasePath:string) {
  ...
  // If the physical transcript changed before this maintenance began, a UI
  // Reset/New lifecycle won the race. Never compact the replacement transcript.
  ...
  // Accept a successor sessionId only when our successful semantic compaction
  // caused it and CNXCLAW generation remained unchanged. Otherwise a UI lifecycle
  // may have replaced the physical session and hard trim is forbidden.
...
export function installContextGuard(api:any,registrationApi:any,config:ContextGuardConfig) {
  ...
  const paths=(ctx:any)=>{const workspaceDir=resolve(config.workspaceDir??ctx?.workspaceDir??ctx?.config?.agents?.defaults?.workspace??process.cwd());
    return {workspaceDir,databasePath:resolve(config.ticketDatabasePath??resolve(workspaceDir,".cogentnexus-openclaw","runtime","cogentnexus-openclaw.sqlite3"))};};
  ...
  registrationApi.registerService?.({id:"cogentnexus-openclaw-context-maintenance-v090",start:async(ctx:any)=>{
    ...
    const tick=async()=>{if(active)return;active=true;try{
      for(const row of due(databasePath,maxAttempts)){
        ...
        try{const result=await maintain(api,row,config,workspaceDir,databasePath);api.logger.info?.(`CogentNexus-OpenClaw context maintenance ${row.session_key}: ${result.action}`);}
        catch(error){const message=error instanceof Error?error.message:String(error),exhausted=attempt>=maxAttempts;
          ...
          api.logger.warn?.(`CogentNexus-OpenClaw context maintenance failed for ${row.session_key} attempt ${attempt}/${maxAttempts}: ${message}`);}
      }}finally{active=false;}};
    // Delay first maintenance slightly so v0.9 owner/native startup fences finish
    // before a crash-recovered, human-authorized compaction row can invoke a model.
    initial=setTimeout(()=>{void tick();},1000);initial.unref?.();
    interval=setInterval(()=>{void tick();},Math.max(1000,Math.min(config.contextMaintenancePollMs??3000,30000)));interval.unref?.();
  },stop:async()=>{if(initial)clearTimeout(initial);if(interval)clearInterval(interval);initial=undefined;interval=undefined;}});
}
