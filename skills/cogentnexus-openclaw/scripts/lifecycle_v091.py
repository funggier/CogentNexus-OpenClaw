#!/usr/bin/env python3
"""CogentNexus-OpenClaw destructive lifecycle commands for cnxclaw.cmd.
...
OpenClaw first, then removes CogentNexus-OpenClaw-owned state, plugin registration,
startup integration, skill files, and the cnxclaw.cmd launcher.
...
PLUGIN_ID = "cogentnexus-openclaw"
...
def local_cnx_data_root() -> Path | None:
    ...
    if not value:
        return None
    return Path(value).expanduser().resolve() / "CogentNexus-OpenClaw"
...
def node_executable() -> str:
    ...
    if not found:
        raise FileNotFoundError("Node.js is required for CogentNexus-OpenClaw schema bootstrap")
    return found


def confirm(action: str) -> bool:
    if action == "reset":
        print("WARNING: CogentNexus-OpenClaw will be reset to fresh-install state.")
        print("")
        print("This permanently removes CogentNexus-OpenClaw Tickets, recovery/delivery/runtime state,")
        print("session authority, workflow runtime data, diagnostics, and CNXCLAW configuration changes.")
        print("Installed CogentNexus-OpenClaw program files and release version remain unchanged.")
        print("OpenClaw and Ollama data are not removed.")
    elif action == "uninstall":
        print("WARNING: This will completely remove CogentNexus-OpenClaw.")
        print("")
        print("CogentNexus-OpenClaw runtime state, configuration, startup integration, OpenClaw plugin,")
        print("skill files, backups, and cnxclaw.cmd will be removed.")
    ...
    try:
        ...
        answer = ""
    if answer != "y":
        print(f"CogentNexus-OpenClaw {action} cancelled. No changes were made.")
        return False
    return True
...
def disable_managed(root: Path) -> None:
    ...
    if result.returncode != 0:
        raise RuntimeError("cnxclaw disable failed; refusing destructive lifecycle while CNXCLAW ownership may still be MANAGED")
...
def bootstrap_ticket_database() -> None:
    if not BOOTSTRAP.is_file():
        raise RuntimeError(
            "installed CogentNexus-OpenClaw plugin lacks scripts/bootstrap-ticket-db.mjs; "
...
def verify_plugin_loaded() -> dict[str, Any]:
    ...
    if plugin.get("enabled") is not True or plugin.get("activated") is not True or plugin.get("status") != "loaded":
        raise RuntimeError(
            "CogentNexus-OpenClaw plugin did not return to loaded MANAGED runtime after reset: "
...
def uninstall_plugin() -> None:
    ...
    if plugin_registered():
        ...
        forward(result)
        if result.returncode != 0:
            raise RuntimeError("OpenClaw CogentNexus-OpenClaw plugin uninstall failed")
    remove_linked_load_paths()
    config_unset(f"plugins.entries.{PLUGIN_ID}")
    if plugin_registered():
        raise RuntimeError("CogentNexus-OpenClaw plugin remains registered after uninstall cleanup")
...
def uninstall(root: Path) -> int:
    ...
    try:
        ...
        if os.name == "nt":
            # cnxclaw.cmd and this Python module may still be executing. Delete them
