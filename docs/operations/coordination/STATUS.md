...
v0.9.7 development continues under CNX-444. Confirmed Gateway force-stop/classification and owner-session liveness now work physically. The next OpenClaw 2026.9.5 compatibility failure was traced to the legacy JSONL embedded-recovery transcript target; local RED-to-GREEN now uses detached embedded execution with the internal admission fence preserved. The next gate is a new exact candidate + exact-SHA CI + live install-over + fresh exactly-once recovery acceptance. v0.9.6 remains the published accepted baseline.
