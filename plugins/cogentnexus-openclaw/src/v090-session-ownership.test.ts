...
describe("v0.9 session ownership isolation", () => {
  ...
  it("Reset creates a new generation on the same session key without touching another session", () => {
    ...
    try {
      ...
      expect(db.prepare("SELECT count(*) AS count FROM ticket_events WHERE ticket_id=? AND event_type='cancelled_by_session_reset'").get(a.ticketId)).toEqual({count:1});
      db.close();
    } finally { rmSync(root,{recursive:true,force:true}); }
  });
  ...
  it("Delete tombstones one session, preserves its durable history, and does not transfer ownership", () => {
    ...
    try {
      ...
      const db = new DatabaseSync(path,{readOnly:true});
      expect(db.prepare("SELECT owner_session_key,status FROM tickets WHERE ticket_id=?").get(a.ticketId)).toEqual({owner_session_key:"agent:main:dashboard:A",status:"cancelled"});
      expect(db.prepare("SELECT owner_session_key,status FROM tickets WHERE ticket_id=?").get(b.ticketId)).toEqual({owner_session_key:"agent:main:dashboard:B",status:"accepted"});
      db.close();
      expect(() => store.accept({runId:"a-new",ownerSessionKey:"agent:main:dashboard:A",prompt:"must not revive"})).toThrow(/session is deleted/i);
    } finally { rmSync(root,{recursive:true,force:true}); }
  });
  ...
  it("rejects late assistant delivery from a superseded session generation", () => {
    ...
    try {
      ...
      expect(queueAssistantDelivery(path,{ticketId:ticket.ticketId,ownerSessionKey:"agent:main:dashboard:A",ownerGeneration:generation,kind:"notice",text:"old result",target:{kind:"notice"},idempotencyKey:"old-result"})).toBe(true);
      cancelSessionByKey(path,{sessionKey:"agent:main:dashboard:A",message:"stop"});
      expect(queueAssistantDelivery(path,{ticketId:ticket.ticketId,ownerSessionKey:"agent:main:dashboard:A",ownerGeneration:generation,kind:"notice",text:"late result",target:{kind:"notice"},idempotencyKey:"late-result"})).toBe(false);
      const db = new DatabaseSync(path,{readOnly:true});
      expect(db.prepare("SELECT count(*) AS count FROM cnx_assistant_delivery WHERE status='pending'").get()).toEqual({count:0});
      db.close();
    } finally { rmSync(root,{recursive:true,force:true}); }
  });
  ...
  it("does not execute a scheduled synthetic turn captured before Reset", async () => {
    ...
    try {
      ...
      let runs = 0;
      const api = {runtime:{subagent:{getSessionMessages:async()=>({messages:[]}),run:async()=>{runs++;return{runId:"must-not-run"};},waitForRun:async()=>({status:"ok"}),deleteSession:async()=>{}},tasks:{}},logger:{warn:()=>{}}};
      const result = await executeCompatibilityWake(api,{workspaceDir:root,ticketDatabasePath:path},{sessionKey:"agent:main:dashboard:A",ownerGeneration:oldGeneration,delayMs:0,deleteAfterRun:true,deliveryMode:"announce",name:"stale",tag:"stale-before-reset",message:"old scheduled internal work"} as any);
      expect(runs).toBe(0);
  ...
  it("reactivates a genuinely new lifecycle on the same deleted session key", () => {
    ...
    try {
      ...
      expect(reactivate(path,{sessionKey:key,sessionId:"A"})).toMatchObject({state:"deleted",accepted:false,lifecycleMatches:false});
      ...
      expect((v090 as any).reactivateSessionForLifecycle(path,{sessionKey:key,sessionId:"B"})).toMatchObject({state:"active",generation:deleted.generation});
      expect((v090 as any).reactivateSessionForLifecycle(path,{sessionKey:key,sessionId:"A"})).toMatchObject({state:"active",accepted:false,lifecycleMatches:false});
      expect((v090 as any).reactivateSessionForLifecycle(path,{sessionKey:key,sessionId:"C"})).toMatchObject({state:"active",accepted:false,lifecycleMatches:false});
      expect((v090 as any).reactivateSessionForLifecycle(path,{sessionKey:key,sessionId:"B"})).toMatchObject({state:"active",accepted:true,lifecycleMatches:true,generation:deleted.generation});
      const identity = new DatabaseSync(path,{readOnly:true});
      expect(identity.prepare("SELECT generation,session_id,state FROM cnx_sessions WHERE session_key=?").get(key)).toEqual({generation:deleted.generation,session_id:"B",state:"active"});
      identity.close();
  ...
  it("blocks before_agent_run when OpenClaw lifecycle identity is stale", async () => {
    ...
    try {
      ...
      const api:any={pluginConfig:{ticketFirst:false,preInferenceAdmission:false,ticketDatabasePath:path,workspaceDir:root},registerTool:()=>{},registerService:()=>{},on:(name:string,handler:any)=>{const list=hooks.get(name)??[];list.push(handler);hooks.set(name,list);},logger:{warn:()=>{},error:()=>{},info:()=>{}},session:{workflow:{}},runtime:{tasks:{managedFlows:{}}}};
      (v090Entry as any).register(api);
      const before=hooks.get("before_agent_run")??[];
      const stale=before.map(handler=>handler({prompt:"ordinary owner prompt"},{sessionKey:key,sessionId:"stale-A",workspaceDir:root})).find((result:any)=>result?.category==="cnxclaw_lifecycle_identity");
      expect(stale).toMatchObject({outcome:"block",category:"cnxclaw_lifecycle_identity"});
      const current=before.map(handler=>handler({prompt:"ordinary owner prompt"},{sessionKey:key,sessionId:"current-B",workspaceDir:root})).find((result:any)=>result?.category==="cnxclaw_lifecycle_identity");
  ...
  it("admits the first owner turn before asynchronous session_start", () => {
    ...
    try {
      ...
      const verify=new DatabaseSync(path,{readOnly:true});
      expect(verify.prepare("SELECT state,session_id,generation FROM cnx_sessions WHERE session_key=?").get(key)).toEqual({state:"active",session_id:"B",generation:deleted.generation});
      verify.close();
