      ...
      const run=async()=>{
        ...
        try {
          do {
            ...
            if(reset>0)api.logger.warn?.(`CogentNexus-OpenClaw reset ${reset} stale Direct recovery claim(s)`);
            ...
            if(recovery) {
              void launchV093DirectRecovery(api,path,workspace,recovery,cfg)
                .catch((error)=>api.logger.warn?.(`CogentNexus-OpenClaw Direct recovery launch failed: ${error instanceof Error?error.message:String(error)}`))
                .finally(()=>queueMicrotask(pulse));
            }
          } while(rerun&&!stopped);
        } catch(error) {
          api.logger.warn?.(`CogentNexus-OpenClaw Direct recovery event worker failed: ${error instanceof Error?error.message:String(error)}`);
        } finally {
          active=false;
          arm();
        }
