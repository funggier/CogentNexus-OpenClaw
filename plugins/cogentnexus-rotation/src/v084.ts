...
const PATCH=Symbol.for("cogentnexus.v084.ticket-patch"), WRAP=Symbol.for("cogentnexus.v084.entry-wrap");
...
function resetCompletions(workspace:string){const root=resolve(workspace,".cogent","workflows");if(!existsSync(root))return 0;let count=0;for(const e of readdirSync(root,{withFileTypes:true})){if(!e.isDirectory())continue;const path=join(root,e.name,"completion.json");if(!existsSync(path))continue;try{const n=JSON.parse(readFileSync(path,"utf8"));if(n?.deliveryStatus!=="pending"||(!n.scheduledAt&&!n.deliveryRunId))continue;delete n.scheduledAt;delete n.deliveryRunId;const tmp=`${path}.${process.pid}.v084.tmp`;writeFileSync(tmp,`${JSON.stringify(n,null,2)}\n`);renameSync(tmp,path);count++;}catch{}}return count;}
export function prepareV084RecoveryState(workspace:string,cfg:Cfg={}){
  const path=dbPath(cfg,workspace),db=openDb(path),stamp=now();let reopened=0,outboxReset=0;
  try{db.exec("BEGIN IMMEDIATE");const rows=db.prepare("SELECT ticket_id,prompt,status FROM tickets WHERE status IN ('waiting','failed') AND workflow_eligible=1 AND workflow_id IS NULL AND failure_class='interrupted' ORDER BY created_at").all() as any[];
    for(const r of rows){if(classifyDurableRequest(r.prompt,cfg.admissionMinimumScore??5).lane!=="direct")continue;db.prepare("DELETE FROM ticket_outbox WHERE ticket_id=? AND delivery_status='pending'").run(r.ticket_id);const c=db.prepare("UPDATE tickets SET status='accepted',workflow_eligible=0,worker_id=NULL,lease_token=NULL,lease_expires_at=NULL,heartbeat_at=NULL,response_ready_at=NULL,delivery_confirmed_at=NULL,updated_at=? WHERE ticket_id=? AND workflow_id IS NULL").run(stamp,r.ticket_id);if(c.changes){queueRecovery(db,r.ticket_id,"resume",`v0.8.4 reopened ${r.status} Direct Ticket`,stamp);addEvent(db,r.ticket_id,"v084_direct_recovery_reopened",{previousStatus:r.status},stamp);reopened++;}}
  ...
  return{databasePath:path,reopened,outboxReset,workflowDeliveryReset:resetCompletions(workspace)};
}
