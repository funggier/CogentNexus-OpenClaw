    ...
    def test_interrupted_promotion_requires_fresh_identified_session(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            sessions = [
                ...
                ("agent:main:test:ambiguous", "active", 1, "2026-08-18T09:50:00Z", "2026-08-18T09:55:00Z", None),
                ("agent:main:test:fresh", "active", 2, "2026-08-18T09:50:00Z", "2026-08-18T09:55:00Z", "fresh-session"),
            ]
            db.executemany("INSERT INTO cnx_sessions VALUES (?,?,?,?,?,?)", sessions)
            tickets = [
                ("T-stale", "run-stale", sessions[0][0]),
                ("T-ambiguous", "run-ambiguous", sessions[1][0]),
                ("T-fresh", "run-fresh", sessions[2][0]),
            ]
            db.executemany(
            ...
            check.close()
            self.assertEqual(states, {"T-ambiguous": 0, "T-fresh": 1, "T-stale": 0})
