#!/usr/bin/env python3
"""Deterministic CogentNexus-OpenClaw workflow runner (Phase 4 core)."""
import argparse
import copy
import hashlib
import json
...
class Workflow:
    def __init__(self, root, task_id):
        ...
        self.base = self.root / ".cogentnexus-openclaw" / "workflows" / task_id
...
def rebind_session_owner(root, from_session_key, to_session_key):
    base = Path(root).resolve() / ".cogentnexus-openclaw" / "workflows"
...
def discover_workflows(root):
    base = Path(root).resolve() / ".cogentnexus-openclaw" / "workflows"; found = []
...
def parser():
    p=argparse.ArgumentParser(description="CogentNexus-OpenClaw deterministic workflow engine"); p.add_argument("--root",type=Path,default=Path.cwd()); sub=p.add_subparsers(dest="command",required=True)
...
def main():
    ...
    if conversational and ((args.command == "init" and args.operator_unbound) or args.command == "bind-owner"):
        raise SystemExit("CogentNexus-OpenClaw Enforced Mode: conversational workflows must use cnxclaw_workflow_start with trusted owner binding")
