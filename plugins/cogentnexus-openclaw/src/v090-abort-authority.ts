...
export const RECOVERABLE_ABORT_MESSAGE = "CogentNexus-OpenClaw recoverable interruption: OpenClaw abort provenance was not human-authoritative";
  ...
  proxy.on=(name:string,handler:any,options?:any)=>{
    ...
    return originalOn(name,(event:any,ctx:any)=>{
      ...
      const forwarded=(authority==="recoverable-maintenance"||authority==="recoverable-ambiguous")
        ...
        : event;
      if(forwarded!==event)api.logger?.info?.(`CogentNexus-OpenClaw abort authority: ${authority}; preserving run as recoverable interruption`);
