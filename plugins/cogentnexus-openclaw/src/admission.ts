...
export function classifyDurableRequest(prompt:string,minimumScore=5):AdmissionDecision{
  ...
  if (/\[(?:CogentNexus-OpenClaw|Subagent) Context\]|cogent-workflow-result-|#cogent-direct\b/iu.test(prompt)) return {lane:"direct",score:0,reasons:["internal-or-explicit-direct"],sections:[],domain};
...
export function compileDurableIntake(input:{workspaceDir:string;prompt:string;runId:string;decision:AdmissionDecision;model:string}){
  const requestHash=durableRequestFingerprint(input.prompt),taskId=`CNX-AUTO-${safeId(input.runId)}`,relativeBase=`.cogentnexus-openclaw/intake/${taskId}`,base=resolve(input.workspaceDir,relativeBase);
