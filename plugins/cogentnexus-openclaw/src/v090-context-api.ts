...
function parseJsonOutput(stdout:string) {
  ...
  try{return JSON.parse(value);}catch{throw new Error(`CogentNexus-OpenClaw Host context adapter returned invalid JSON: ${value.slice(0,500)}`);}
}
...
async function runHostCompact(api:any,cfg:any,params:any,options:any) {
  ...
  const root=resolve(cfg?.cogentNexusOpenClawRoot ?? resolve(workspace,".cogentnexus-openclaw"));
  const script=resolve(workspace,"skills","cogentnexus-openclaw","scripts","host_context.py");
...
/**
 * Context maintenance facade for CogentNexus-OpenClaw v0.9.
 *
