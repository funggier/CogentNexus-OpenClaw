# Provider Lifecycle — CogentNexus-OpenClaw v0.9.6
...
Provider/model selection itself remains an OpenClaw routing concern. Legacy `--provider` input that implied CNX provider-routing authority is not part of the current v0.9.6 contract.
...
Provider ownership does not override session Stop authority. The v0.9.6 pre-dispatch FIFO barrier prevents a cancelled held message from reaching the Host queue after a user Stop, regardless of provider.
