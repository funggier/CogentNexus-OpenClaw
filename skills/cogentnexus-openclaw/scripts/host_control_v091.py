#!/usr/bin/env python3
"""CogentNexus-OpenClaw v0.9.1 Host control shim with symmetric enable rollback.
...
Destructive installation lifecycle commands remain part of cnxclaw.cmd itself:
`reset` recreates fresh state for the installed release, while `uninstall`
restores native OpenClaw and removes CogentNexus-OpenClaw. Both require explicit `y`.
"""
...
def _repair_managed_plugin_activation(root: Path, execute_safe: bool) -> dict[str, Any] | None:
    ...
    if plugin.get("enabled") is not True or plugin.get("activated") is not True or plugin.get("status") != "loaded":
        raise RuntimeError(
            "CogentNexus-OpenClaw plugin failed runtime activation verification after MANAGED repair: "
...
def main() -> int:
    ...
    # reset/uninstall are cnxclaw.cmd lifecycle commands, not Host state-machine
    ...
    try:
        ...
        print(json.dumps({
            "result": "error",
            "error": f"CogentNexus-OpenClaw watchdog compatibility failed before enable: {error}",
        }, ensure_ascii=False, indent=2))
        return 1
    ...
    if rollback_error:
        print(json.dumps({
            "result": "error",
            "error": "CogentNexus-OpenClaw enable failed and watchdog compatibility rollback also failed",
            "delegateExitCode": code,
            "rollbackError": rollback_error,
        }, ensure_ascii=False, indent=2))
    return code
