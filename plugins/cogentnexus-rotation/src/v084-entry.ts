        ...
        const tick = () => {
          ...
          try {
            ...
            if (result.reopened > 0) {
              api.logger.info?.(`CogentNexus v0.8.4 reconciled ${result.reopened} Host-promoted Direct Ticket(s) back to Direct recovery`);
            }
          } catch (error) {
            api.logger.warn(`CogentNexus v0.8.4 Host reconciliation failed: ${error instanceof Error ? error.message : String(error)}`);
          } finally {
            active = false;
          }
        };
