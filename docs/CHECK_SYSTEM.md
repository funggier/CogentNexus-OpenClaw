...
CogentNexus-OpenClaw v0.9.8 groups diagnostic inspection under the `check` namespace. The current managed provider contract is **Ollama only**.
...
Current v0.9.8 managed-provider checks target Ollama:
...
The report is observational only. Unsupported provider names are outside the v0.9.8 managed contract.
...
Frozen v0.9.2 source/evidence may describe provider-neutral checks, including LM Studio. That history remains valid historical evidence, but it is not current v0.9.8 operator guidance.
