#!/usr/bin/env python3
"""CogentNexus-OpenClaw v0.9.2 Host control shim.
...
def _finish_disable_native_boundary(root: Path, delegate_code: int) -> int:
    ...
    if not adapter_stop.get("ok"):
        print(json.dumps({
            ...
            "safety": "CogentNexus-OpenClaw is PASSTHROUGH, but native route activation is blocked until the provider event adapter releases ownership",
    ...
    if not restored.get("ok"):
        print(json.dumps({
            ...
            "providerEventAdapterStop": adapter_stop,
            "safety": "CogentNexus-OpenClaw is PASSTHROUGH; destructive cleanup is blocked until the native route is restored",
        }, ensure_ascii=False, indent=2))
        return 1
    ...
    if not boundary.get("ok"):
        print(json.dumps({
            ...
            "safety": "CogentNexus-OpenClaw is PASSTHROUGH and native config is durable, but Gateway reload/health could not be verified",
        }, ensure_ascii=False, indent=2))
        return 1
    ...
    if command == "stop":
        ...
        if not adapter_stop.get("ok"):
            print(json.dumps({
                ...
                "phase": "stop-provider-event-adapter",
                "providerEventAdapterStop": adapter_stop,
                "safety": "provider shutdown was not attempted because the CNXCLAW event adapter could not be verified stopped",
            }, ensure_ascii=False, indent=2))
            return 1
        return v091.main()
    if command == "disable":
