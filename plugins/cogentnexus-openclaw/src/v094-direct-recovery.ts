...
function appendRuntimeErrorDiagnostic(
  ...
  try {
    const dir = join(workspace, ".cogentnexus-openclaw", "runtime");
...
function kickHostDelivery(workspace: string, cfg: V094DirectRecoveryConfig) {
  const preferred = resolve(workspace, "skills", "cogentnexus-openclaw", "scripts", "host_delivery_v092.py");
  const fallback = resolve(workspace, "skills", "cogentnexus-openclaw", "scripts", "host_delivery.py");
...
export async function launchV094DirectRecovery(
  ...
  const runId = `cnxclaw-direct-${recovery.ticket_id.replace(/[^A-Za-z0-9_-]/g, "-").slice(0, 48)}-${attempt}-g${recovery.owner_generation}`;
