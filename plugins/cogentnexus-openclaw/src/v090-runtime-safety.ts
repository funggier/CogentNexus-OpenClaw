...
export async function verifyCnxCompactionResult(input:{originalGateway:any;params:any;result:any}):Promise<any> {
  ...
  if(freshAfter!==undefined&&window!==undefined) {
    ...
    if(freshAfter>limit)return {...result,ok:false,compacted:false,error:`CogentNexus-OpenClaw hard-trim verification remained above safe context target (${freshAfter}/${window})`,
  ...
  return {...result,ok:false,compacted:false,error:"CogentNexus-OpenClaw could not verify hard-trim occupancy or retained-line bound",
...
function requireClaim(databasePath:string,ticketId:string,generation:number,runId:string) {
  ...
  if(!identity.authorized)throw new Error(`CogentNexus-OpenClaw Direct Recovery claim revoked (${identity.reason??"unknown"})`);
  return identity;
}
...
async function waitForContextRelease(input:{databasePath:string;hiddenSessionKey:string;expectedRunId:string;config:RuntimeSafetyConfig;logger:any}) {
  ...
  while(true) {
    ...
    if(Date.now()-lastHeartbeat>=5000){if(!touchDirectRecoveryClaim(input.databasePath,{ticketId,ownerSessionKey:identity.ownerSessionKey,ownerGeneration:generation,runId:input.expectedRunId}))throw new Error("CogentNexus-OpenClaw Direct Recovery claim superseded during context hold");lastHeartbeat=Date.now();}
    const snapshot=contextRecoveryHoldSnapshot(input.databasePath,input.hiddenSessionKey);
    if(snapshot.revoked)throw new Error(`CogentNexus-OpenClaw Direct Recovery authority revoked while waiting for context maintenance (${snapshot.state??"unknown"}/${snapshot.action??""})`);
    if(!snapshot.hold)return;
    if(!announced){announced=true;input.logger.info?.(`CogentNexus-OpenClaw holding hidden Direct Recovery ${ticketId} until context maintenance leaves state=${snapshot.state}`);}
    if(Date.now()>=deadline)throw new Error(`CogentNexus-OpenClaw context recovery hold timed out for ${ticketId}`);
...
async function waitForRecoveryOrder(input:{databasePath:string;hiddenSessionKey:string;expectedRunId:string;config:RuntimeSafetyConfig;logger:any}) {
  ...
  while(true) {
    const identity=requireClaim(input.databasePath,ticketId,generation,input.expectedRunId);
    if(Date.now()-lastHeartbeat>=5000){if(!touchDirectRecoveryClaim(input.databasePath,{ticketId,ownerSessionKey:identity.ownerSessionKey,ownerGeneration:generation,runId:input.expectedRunId}))throw new Error("CogentNexus-OpenClaw Direct Recovery claim superseded during ordered wait");lastHeartbeat=Date.now();}
    ...
    if(announcedPredecessor!==String(predecessor.ticket_id)){announcedPredecessor=String(predecessor.ticket_id);input.logger.info?.(`CogentNexus-OpenClaw holding Direct Recovery ${ticketId} behind ${announcedPredecessor} (${String(predecessor.state)}) in ${identity.ownerSessionKey}`);}
    await new Promise((resolvePromise)=>setTimeout(resolvePromise,pollMs));
  }
}
...
export function createCnxRuntimeSafetyProxy(api:any,config:RuntimeSafetyConfig={}) {
  ...
  if(originalSubagent?.run) {
    ...
    subagent.run=async(input:any)=>{
      ...
      if(!sessionKey.includes(":subagent:cnx-")||!/\[CogentNexus-OpenClaw Internal/iu.test(message))return originalSubagent.run(input);
      if(/\[CogentNexus-OpenClaw Internal Direct Recovery\]/iu.test(message)) {
        ...
        if(ticketId) {
          ...
          if(!expectedRunId)throw new Error(`CogentNexus-OpenClaw Direct Recovery ${ticketId} is missing its claim fencing token`);
          ...
          if(generation===undefined)throw new Error(`CogentNexus-OpenClaw Direct Recovery ${ticketId} has no generation fence`);
      ...
      if(bounded.externalized)api.logger.info?.(`CogentNexus-OpenClaw externalized oversized hidden payload for ${sessionKey}: chunks=${bounded.chunkCount} sha256=${bounded.sha256?.slice(0,16)}`);
