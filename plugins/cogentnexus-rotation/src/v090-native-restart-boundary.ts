 ...
 * human-intent boundary while CNX managed policy is active.
 *
...
export function installNativeRestartRecoveryBoundary(api: any) {
  ...
  api.on?.("before_agent_run", (event: any, ctx: any) => {
    ...
    return {
      outcome:"block",
      reason:"OpenClaw internal main-session restart recovery is not fresh human intent; CogentNexus owns durable recovery",
