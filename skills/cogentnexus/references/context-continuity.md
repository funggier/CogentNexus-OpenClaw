...
For Direct Recovery, reconstruct only the minimum owner-session context required while preserving original Ticket prompt/model provenance. A native restart continuation that merely repeats the same durable prompt is not new user intent when CNX already owns that recovery.
