    ...
    if (tag.startsWith("cogent-resume-")) {
      try {
        if (hostRecoveryOwnsResumeTag(databaseFor(api), tag)) {
          api.logger?.info?.(`CogentNexus-OpenClaw suppressed legacy auto-resume ${tag}: Host Direct model-call recovery owns classification`);
        ...
        api.logger?.warn?.(`CogentNexus-OpenClaw failed closed while checking Host Direct recovery before ${tag}: ${error instanceof Error ? error.message : String(error)}`);
        return Promise.resolve({
          scheduled: false,
          suppressed: true,
...
export function installV091DirectModelCallLease(api: any) {
  ...
  catch (error) { throw new Error(`CogentNexus-OpenClaw Direct model-call lease schema failed: ${error instanceof Error ? error.message : String(error)}`); }
