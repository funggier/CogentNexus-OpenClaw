...
All functions in this module are observational. They must not start/stop/restart
processes, mutate durable state, repair databases, or rewrite configuration.
"""
...
def check_cogentnexus(root: Path) -> list[dict[str, Any]]:
    ...
    required = ["host.py", "runtime.py", "startup.py", "provider.py", "checks.py", "cnx.py"]
    missing = [name for name in required if not (here / name).is_file()]
    results = []
    if missing:
        results.append(item("CogentNexus installation", "FAIL", "Required CogentNexus files are missing", missing=missing))
    else:
        results.append(item("CogentNexus installation", "PASS", "Core/provider/check scripts are present"))
    ...
    if error:
        ...
        results.append(item(
            ...
            "PASS",
            f"Mode is {state.get('mode')}",
            mode=state.get("mode"),
            selectedProvider=state.get("selectedProvider"),
        ...
        if state.get("providerTransition"):
            results.append(item(
                "Provider transition",
                "WARN",
                "A provider transition marker is present; the previous switch may have been interrupted",
...
def check_config(root: Path) -> list[dict[str, Any]]:
    ...
    if not executable:
        ...
        if result.get("ok"):
            results.append(item("OpenClaw config", "PASS", "OpenClaw configuration validates"))
        else:
            results.append(item("OpenClaw config", "FAIL", "OpenClaw configuration validation failed", evidence=result))
    return results
...
def check_openclaw() -> list[dict[str, Any]]:
    ...
    version = _run([executable, "--version"], timeout=20)
    if version.get("ok"):
        return [item("OpenClaw installation", "PASS", "OpenClaw CLI is available", executable=executable, version=version.get("stdout"))]
    return [item("OpenClaw installation", "FAIL", "OpenClaw CLI exists but version check failed", executable=executable, evidence=version)]
...
def check_gateway() -> list[dict[str, Any]]:
    ...
    healthy = result.get("ok") and "connectivity probe: ok" in output and not any(
        marker in output for marker in ("connectivity probe: failed", "runtime: stopped", "econnrefused")
    )
    if healthy:
...
def _resolve_provider(root: Path, override: str | None) -> tuple[str | None, list[dict[str, Any]]]:
    ...
    if override:
        ...
        notes.append(item(
            "Provider selection",
            "PASS",
            f"Preflight requested provider: {requested}",
            requested=requested,
            persisted=selected,
            mutatesState=False,
        ))
    ...
    if selected:
        ...
        notes.append(item("Provider selection", "PASS", f"Using persisted provider: {selected}", selected=selected))
    ...
    if len(installed) == 1:
        notes.append(item(
            "Provider selection",
            "WARN",
            f"No provider is persisted; exactly one provider is installed ({installed[0]})",
            inferred=installed[0],
            mutatesState=False,
        ))
    ...
    return None, [item(
        "Provider selection",
        "FAIL",
        "Multiple providers are installed but no provider has been selected",
        installed=installed,
        suggested=[f"cnx.cmd start --provider {name}" for name in installed],
    )]


def check_provider(root: Path, override: str | None = None, include_inventory: bool = True) -> list[dict[str, Any]]:
    target, results = _resolve_provider(root, override)
    ...
    if include_inventory:
        for name in provider.SUPPORTED_PROVIDERS:
            ...
            summary = (
                f"{name} is installed and reachable"
                if info["healthy"]
                else f"{name} is installed but not currently reachable"
                if info["installed"]
                else f"{name} is not installed"
            )
            results.append(item(
                f"Provider discovery: {name}",
                status,
                summary,
    ...
    if not info["installed"]:
        ...
        results.append(item(
            "Selected provider",
            "WARN",
            f"Provider '{target}' is installed and startable but not currently reachable",
            provider=target,
            endpoint=info["endpoint"],
        ))
    else:
        results.append(item(
            "Selected provider",
            "FAIL",
            f"Provider '{target}' is installed but CogentNexus cannot control it because its CLI adapter is unavailable",
            provider=target,
            endpoint=info["endpoint"],
        ))
    return results
...
def check_model(root: Path, override: str | None = None) -> list[dict[str, Any]]:
    ...
    if not model_ref:
        ...
        results.append(item(
            ...
            "FAIL",
            f"OpenClaw default model routes to '{routed}' while preflight provider is '{target}'",
        ...
        results.append(item(
            "OpenClaw model routing",
            "PASS" if model_ref else "WARN",
            f"OpenClaw default model: {model_ref}" if model_ref else "OpenClaw default model unresolved",
            defaultModel=model_ref,
...
def check_storage(root: Path) -> list[dict[str, Any]]:
    results = []
    root.mkdir(parents=True, exist_ok=True) if False else None  # explicit no-op: checks never create paths
    ...
    if not db_path.exists():
        ...
        try:
            ...
            try:
                ...
                if integrity == "ok" and "tickets" in tables:
                    results.append(item("Ticket store", "PASS", "Ticket database is readable and integrity_check is ok", path=str(db_path)))
                else:
                    results.append(item("Ticket store", "FAIL", "Ticket database integrity/schema check failed", integrity=integrity, tables=sorted(tables)))
            finally:
                db.close()
        except Exception as exc:
    ...
    try:
        ...
        status = "PASS" if usage.free >= 512 * 1024 * 1024 else "FAIL"
        results.append(item("Disk headroom", status, f"Free disk: {usage.free / 1024**3:.2f} GiB", freeBytes=usage.free))
    except Exception as exc:
    ...
    if health.exists():
        try:
            ...
            results.append(item(
                ...
                "PASS" if status == "healthy" else "WARN",
                f"Last supervisor status: {status}",
    ...
    try:
        ...
        try:
            ...
            return [item(
                ...
                "No pending terminal deliveries" if pending == 0 else f"{pending} terminal delivery item(s) are pending",
                pending=pending,
            )]
        finally:
            db.close()
        ...
        if exe:
            ...
            if result.get("ok"):
                try:
                    ...
                    return {
                        "availableBytes": int(value["FreePhysicalMemory"]) * 1024,
...
def check_resources(root: Path) -> list[dict[str, Any]]:
    ...
    status = "PASS" if available >= minimum else "WARN"
    return [item(
        "Memory headroom",
        status,
        f"Available memory: {available / 1024**3:.2f} GiB",
        availableBytes=available,
        minimumFreeBytes=int(minimum),
...
def system_check(root: Path, provider_override: str | None = None) -> dict[str, Any]:
    ...
    checks.extend(check_cogentnexus(root))
    checks.extend(check_config(root))
    checks.extend(check_openclaw())
    checks.extend(check_provider(root, provider_override, include_inventory=True))
    checks.extend(check_model(root, provider_override))
    checks.extend(check_gateway())
    checks.extend(check_storage(root))
    checks.extend(check_recovery(root))
    checks.extend(check_delivery(root))
...
def component_check(root: Path, component: str, provider_override: str | None = None) -> dict[str, Any]:
    root = root.resolve()
    mapping = {
        "cogentnexus": lambda: check_cogentnexus(root),
        ...
        "model": lambda: check_model(root, provider_override),
        "storage": lambda: check_storage(root),
        "recovery": lambda: check_recovery(root),
        "delivery": lambda: check_delivery(root),
        "resources": lambda: check_resources(root),
    }
    if component not in mapping:
        raise ValueError(f"unsupported check component: {component}")
    checks = mapping[component]()
    verdict = aggregate(checks)
    return {
        "check": component,
        "providerOverride": provider_override,
...
def preflight_start(root: Path, provider_name: str) -> dict[str, Any]:
    """Start preflight using the same check primitives, but tolerating stopped services.

    A provider that is installed+controllable but currently stopped is startable and
    therefore passes preflight. No mutation occurs here.
    """
...
def render(report: dict[str, Any]) -> str:
    ...
    for entry in report.get("checks", []):
        ...
        lines.append(f"[{status}] {entry.get('name')}")
        lines.append(f"       {entry.get('summary')}")
    lines.extend([
        "-" * 58,
        f"SYSTEM READINESS: {report.get('verdict')}",
