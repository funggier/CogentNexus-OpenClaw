...
function fixture() {
  ...
  dirs.push(dir);
  const path = join(dir, "cogentnexus.sqlite3");
...
describe("v0.9.9 native restart ownership fence", () => {
  ...
  it("consumes exact native restart dispatch when CNX owns recovery", () => {
    ...
    installV099NativeRestartOwnershipFence(api, {
      workspaceDir: dir,
      ticketDatabasePath: join(dir, "cogentnexus.sqlite3"),
    });
  ...
  it("passes ordinary messages even while CNX owns recovery", () => {
    const { dir, sessionKey } = fixture();
    let handler: ((event: any, ctx: any) => any) | undefined;
    const api = {
      config: { agents: { defaults: { workspace: dir } } },
