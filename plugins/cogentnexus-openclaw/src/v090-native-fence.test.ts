...
describe("v0.9 native OpenClaw task fence", () => {
  ...
  it("fences a running CNXCLAW failed-ticket delivery", () => {
    expect(shouldFenceNativeCnxTask({
      id:"task-1",
      label:"plugin:cogentnexus-openclaw",
      runId:"cnx-scheduled-deadbeef",
      status:"running",
      title:"[CogentNexus-OpenClaw Delivery: ticket:5]\nCogentNexus-OpenClaw Ticket CNXT-failed-1 reached terminal status failed.",
    }, failed)).toBe(true);
  });

  it("fences a running CNXCLAW cancelled ticket even without the delivery status line", () => {
    expect(shouldFenceNativeCnxTask({
      id:"task-2",
      label:"plugin:cogentnexus-openclaw",
      runId:"cnxclaw-direct-CNXT-cancelled-1-3-abcdef",
      status:"running",
      title:"Resume CogentNexus-OpenClaw Ticket CNXT-cancelled-1",
    }, cancelled)).toBe(true);
  });
  ...
  it("does not fence completed or recoverable tickets", () => {
    const completedTask = {
      id:"task-3",
      label:"plugin:cogentnexus-openclaw",
      runId:"cnxclaw-direct-CNXT-completed-1-1-abcdef",
      status:"running",
      title:"Resume CogentNexus-OpenClaw Ticket CNXT-completed-1",
    };
    const waitingTask = {
      id:"task-4",
      label:"plugin:cogentnexus-openclaw",
      runId:"cnxclaw-direct-CNXT-waiting-1-1-abcdef",
      status:"running",
      title:"Resume CogentNexus-OpenClaw Ticket CNXT-waiting-1",
    };
  ...
  it("does not touch unrelated OpenClaw tasks", () => {
    expect(shouldFenceNativeCnxTask({
      ...
      runId:"other-run",
      status:"running",
      title:"CogentNexus-OpenClaw Ticket CNXT-failed-1 reached terminal status failed.",
    }, failed)).toBe(false);
  });
  ...
  it("fences blocked workflow delivery but not successful workflow completion", () => {
    expect(shouldFenceNativeCnxTask({
      ...
      status:"queued",
      title:"[CogentNexus-OpenClaw Delivery: workflow:CNX-AUTO-test:12]\nCogentNexus-OpenClaw workflow CNX-AUTO-test reached terminal status blocked.",
    }, new Map())).toBe(true);
    expect(shouldFenceNativeCnxTask({
      id:"task-7",
      label:"plugin:cogentnexus-openclaw",
      ...
      status:"queued",
      title:"[CogentNexus-OpenClaw Delivery: workflow:CNX-AUTO-test:13]\nCogentNexus-OpenClaw workflow CNX-AUTO-test reached terminal status completed.",
    }, new Map())).toBe(false);
  });
