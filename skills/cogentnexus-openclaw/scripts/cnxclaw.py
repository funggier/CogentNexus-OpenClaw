#!/usr/bin/env python3
"""CogentNexus-OpenClaw v0.9.8 operator CLI boundary.
...
def help_text() -> str:
    return """CogentNexus-OpenClaw v0.9.8
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command in {"start", "stop", "restart", "status", "enable", "disable", "reset", "uninstall"} and has_option(args[1:], "--provider"):
        emit({"result": "error", "error": "--provider is no longer a CNX lifecycle authority in v0.9.8; select provider/model in OpenClaw instead", "compatibility": "legacy input rejected and no route/lifecycle transition was attempted"})
        return 2
